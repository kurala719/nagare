import { createI18n } from 'vue-i18n'

const browserLocale = (typeof navigator !== 'undefined' && navigator.language)
  ? (String(navigator.language).toLowerCase().startsWith('zh') ? 'zh' : 'en')
  : 'en'

const savedLocale = (typeof localStorage !== 'undefined' && localStorage.getItem('nagare_locale')) || browserLocale

const messages = {
  en: {
    mcp: {
      title: 'MCP Servers',
      alertInfo: 'Settings are saved locally inside configs/mcp_config.json. Click Save after making changes.',
      serviceName: 'Service Name',
      command: 'Command',
      addTitle: 'Add MCP Server',
      editTitle: 'Edit MCP Server',
      name: 'Name',
      args: 'Arguments',
      argsPlaceholder: 'Type and press Enter to add arguments',
      envVars: 'Environment Variables',
      addEnv: 'Add Env',
      envKey: 'Key',
      envValue: 'Value',
      removedLocally: 'Removed locally. Click Save to persist changes.',
      savedToList: 'Saved to list. Click "Save" at the top to persist.',
      saveSuccess: 'MCP Configurations saved successfully',
      deleteTitle: 'Confirm Delete',
      deleteConfirmText: 'Are you sure you want to remove this MCP Server?',
      testPassed: 'Test passed! Discovered tools: {count}',
      testFailed: 'Test failed: {message}'
    },
    common: {
      language: 'Language',
      day: 'Day',
      night: 'Night',
      back: 'Back',
      refresh: 'Refresh',
      loading: 'Loading...',
      startTime: 'Start time',
      endTime: 'End time',
      noHistoryData: 'No history data',
      historyLoadFailed: 'Failed to load history',
      comparePrevious: 'Compare',
      currentPeriod: 'Current',
      previousPeriod: 'Previous',
      expand: 'Expand',
      collapse: 'Collapse',
      showChat: 'Show Chat',
      hideChat: 'Hide Chat',
      search: 'Search',
      globalSearch: 'Global Search',
      searchPlaceholder: 'Search anything...',
      columns: 'Columns',
      actions: 'Actions',
      add: 'Add',
      test: 'Test',
      status: 'Status',
      confirm: 'Confirm',
      enabled: 'Enabled',
      disabled: 'Disabled',
      statusInactive: 'Inactive',
      statusActive: 'Active',
      statusError: 'Error',
      statusSyncing: 'Syncing',
      reasonInactive: 'Not running or disabled',
      reasonActive: 'Operating normally',
      reasonError: 'Error detected',
      reasonSyncing: 'Synchronization in progress',
      noMore: 'No more data',
      selectedCount: 'Selected: {count}',
      bulkUpdate: 'Bulk Update',
      bulkDelete: 'Bulk Delete',
      bulkUpdateTitle: 'Bulk Update',
      bulkUpdateNoChange: 'No change',
      bulkUpdateNoChanges: 'Please choose at least one field to update',
      bulkUpdateSuccess: 'Updated {count} records',
      bulkUpdateFailed: 'Failed to update records',
      bulkDeleteConfirmTitle: 'Confirm Bulk Delete',
      bulkDeleteConfirmText: 'Delete {count} selected records?',
      bulkDeleteSuccess: 'Deleted {count} records',
      bulkDeleteFailed: 'Failed to delete selected records',
      selectAtLeastOne: 'Please select at least one record',
      selectAll: 'Select All',
      deselectAll: 'Deselect All',
      sort: 'Sort by',
      sortNameAsc: 'Name (A-Z)',
      sortNameDesc: 'Name (Z-A)',
      sortCreatedAsc: 'Oldest created',
      sortCreatedDesc: 'Newest created',
      sortUpdatedAsc: 'Oldest updated',
      sortUpdatedDesc: 'Recently updated',
      sortStatusDesc: 'Status (High-Low)',
      sortStatusAsc: 'Status (Low-High)',
      apply: 'Apply',
      footer: 'Footer',
      ok: 'OK',
      edit: 'Edit',
      delete: 'Delete',
      save: 'Save',
      saveLocally: 'Save Locally',
      saveAndPush: 'Save and Push',
      deleteLocally: 'Delete Locally',
      deleteAndPush: 'Delete and Push',
      cancel: 'Cancel',
      warning: 'Warning',
      invalidJson: 'Invalid JSON',
      invalidEmail: 'Invalid email format',
      weakPassword: 'Password must be at least 8 characters and include 3 of: lowercase, uppercase, digits, special characters',
      invalidUsername: 'Username must be 3-32 characters, alphanumeric with underscores/hyphens only',
      invalidInput: 'Invalid input',
      unauthorizedTitle: 'Unauthorized',
      accessDeniedTitle: 'Access Denied',
      operationFailed: 'Operation failed',
      sessionExpired: 'Your session has expired. Please login again.',
      insufficientPrivileges: 'Insufficient privileges. Please login with an authorized account.',
      loginToContinue: 'Please login to continue.',
      dayUnit: 'd',
      hourUnit: 'h',
      minuteUnit: 'm',
      secondUnit: 's',
      details: 'Log Details',
      clear: 'Clear Logs',
      export: 'Export CSV',
      autoRefresh: 'Auto Refresh',
      rawContext: 'Raw Context',
      formattedContext: 'Formatted Context',
      retry: 'Retry',
      applyAndSave: 'Apply and Save',
      noData: 'No data',
      sync: 'Sync',
      view: 'View',
      unknown: 'Unknown'
    },
    auth: {
      login: 'Login',
      register: 'Register',
      reset: 'Reset Password',
      logout: 'Logout',
      guest: 'Guest',
      signInTitle: 'Sign in to Nagare',
      tagline: 'Deep-dive infrastructure intelligence and automated operations.',
      welcomeBack: 'Welcome Back',
      rememberMe: 'Remember me',
      registerTitle: 'Submit registration application',
      resetTitle: 'Reset password',
      username: 'Username',
      email: 'Email',
      password: 'Password',
      confirmPassword: 'Confirm Password',
      oldPassword: 'Old Password',
      newPassword: 'New Password',
      confirmNewPassword: 'Confirm New Password',
      submit: 'Submit',
      backToLogin: 'Back to login',
      createAccount: 'Create account',
      resetLink: 'Reset password',
      enterCredentials: 'Please enter username and password',
      emailInvalid: 'Please enter a valid email address',
      missingToken: 'Missing token',
      loginFailed: 'Login failed',
      passwordsMismatch: 'Passwords do not match',
      passwordTooWeak: 'Password is too weak',
      passwordRules: 'Password must be at least 8 characters and include 3 of: uppercase, lowercase, number, symbol',
      applicationSubmitted: 'Application submitted. Please wait for approval.',
      resetApplicationSubmitted: 'Reset request submitted. Please wait for admin approval.',
      registrationFailed: 'Registration failed',
      completeAllFields: 'Please complete all fields',
      resetSuccess: 'Password reset. Please login.',
      resetFailed: 'Reset failed',
      submitResetRequest: 'Submit Reset Request'
    },
    registerApplications: {
      title: 'Registration Applications',
      search: 'Search applications',
      filterStatus: 'Filter status',
      filterAll: 'All',
      statusPending: 'Pending',
      statusApproved: 'Approved',
      statusRejected: 'Rejected',
      refresh: 'Refresh',
      loading: 'Loading applications...',
      retry: 'Retry',
      noApplications: 'No applications found',
      id: 'ID',
      username: 'Username',
      status: 'Status',
      reason: 'Reason',
      approvedBy: 'Approved By',
      createdAt: 'Created At',
      actions: 'Actions',
      approve: 'Approve',
      reject: 'Reject',
      approved: 'Application approved',
      rejected: 'Application rejected',
      approveSelected: 'Approve Selected',
      rejectSelected: 'Reject Selected',
      approveFailed: 'Failed to approve',
      rejectFailed: 'Failed to reject',
      rejectTitle: 'Reject Application',
      reasonPlaceholder: 'Provide a reason (optional)',
      cancel: 'Cancel',
      loadFailed: 'Failed to load applications'
    },
    chat: {
      title: 'Chat Assistant',
      subtitle: 'Ask me anything about your infrastructure.',
      selectProvider: 'Select Provider',
      section: 'Chat',
      loadingHistory: 'Loading history...',
      historyLoaded: 'History loaded',
      thinking: 'Thinking...',
      placeholder: 'Ask a question...',
      send: 'Send',
      empty: 'Start a conversation to see messages here.',
      selectProviderWarn: 'Please select a provider first',
      toneLabel: 'Tone',
      toneProfessional: 'Professional',
      toneRoast: 'Roast',
      toolMode: 'Tool mode',
      toolBadge: 'Tools'
    },
    dashboard: {
      title: 'System Dashboard',
      loading: 'Loading dashboard data...',
      alerts: 'Alerts',
      hosts: 'Hosts',
      groups: 'Groups',
      monitors: 'Monitors',
      providers: 'Providers',
      critical: 'Critical',
      online: 'Online',
      active: 'Active',
      recentAlerts: 'Recent Alerts',
      hostStatus: 'Host Status',
      aiProviders: 'AI Providers',
      viewAll: 'View All',
      id: 'ID',
      message: 'Message',
      severity: 'Severity',
      name: 'Name',
      ip: 'IP',
      status: 'Status',
      model: 'Model',
      noAlerts: 'No alerts',
      noHosts: 'No hosts',
      noGroups: 'No groups',
      noProviders: 'No providers',
      onlineLabel: 'Online',
      offlineLabel: 'Offline',
      activeLabel: 'Active',
      inactiveLabel: 'Inactive',
      healthScore: 'Health Score',
      healthGood: 'Healthy',
      healthWarn: 'Degraded',
      healthBad: 'Critical',
      activeMonitors: 'Active Monitors',
      activeGroups: 'Active Groups',
      impactedGroups: 'Impacted Groups',
      activeHosts: 'Active Hosts',
      activeItems: 'Active Items',
      networkMetrics: 'Network Metrics',
      metricsSearch: 'Search metrics',
      loadingMetrics: 'Loading metrics...',
      noMetrics: 'No metrics found',
      value: 'Value',
      units: 'Units',
      updated: 'Updated',
      refresh: 'Refresh',
      refreshAll: 'Refresh All',
      host: 'Host',
      metric: 'Metric',
      lastUpdated: 'Last refreshed',
      summaryLastUpdated: 'Dashboard refreshed',
      healthLoadFailed: 'Failed to load health score',
      metricsLoadFailed: 'Failed to load metrics',
      healthTrendTitle: 'Network Health Trend',
      topologyTitle: 'Network Topology',
      topologyMonitor: 'Inventory Source',
      topologySite: 'Host Group',
      topologyHost: 'End-point Asset',
      impactedLabel: 'Impacted',
      loadingTopology: 'Loading topology...',
      noTopology: 'No topology data',
      topologyLoadFailed: 'Failed to load topology',
      voiceTitle: 'VoiceOps Command',
      voiceStart: 'Start Voice',
      voiceStop: 'Stop Voice',
      voiceListening: 'Listening',
      voiceIdle: 'Idle',
      voiceHint: 'Try: "Check health", "Show alerts", "Find switch", "Open topology"',
      voiceTranscript: 'Transcript',
      voiceAction: 'Action',
      voiceActionHealth: 'Fetch system health',
      voiceActionAlerts: 'Open alerts page',
      voiceActionSwitch: 'Search for switch hosts',
      voiceActionTopology: 'Focus topology graph',
      voiceNoMatch: 'No matching command',
      voiceNotSupported: 'Speech recognition is not supported in this browser.',
      voiceFailed: 'Voice recognition failed. Please try again.',
      voiceHealthResult: 'Health score',
      voiceHealthFailed: 'Failed to fetch health score',
      matrixTitle: 'Matrix Log Stream',
      matrixPause: 'Pause',
      matrixResume: 'Resume'
    },
    systemStatus: {
      title: 'System Health',
      uptime: 'Uptime',
      goroutines: 'Goroutines',
      allocatedMem: 'Allocated Mem',
      cpus: 'CPUs',
      memoryUsage: 'Memory Usage (MB)',
      systemInfo: 'System Information',
      goVersion: 'Go Version',
      totalAlloc: 'Total Alloc',
      systemMemory: 'System Memory',
      gcCount: 'GC Count'
    },
    system: {
      title: 'Configuration',
      configuration: 'Configuration',
      display: 'Display',
      loading: 'Loading configuration...',
      retry: 'Retry',
      systemSettings: 'System Settings',
      systemName: 'System Name',
      ipAddress: 'IP Address',
      availability: 'Availability',
      port: 'Port',
      databaseSettings: 'Database Settings',
      dbVersion: 'Version',
      dbHost: 'Host',
      dbPort: 'Port',
      dbUser: 'Username',
      dbPassword: 'Password',
      dbName: 'Database Name',
      syncSettings: 'Sync Settings',
      syncEnabled: 'Enabled',
      syncInterval: 'Interval (seconds)',
      syncConcurrency: 'Concurrency',
      statusCheckSettings: 'Status Check Settings',
      statusCheckEnabled: 'Enabled',
      statusCheckInterval: 'Interval (seconds)',
      statusCheckProviderEnabled: 'Auto-check Providers',
      statusCheckConcurrency: 'Concurrency',
      mcpSettings: 'MCP Settings',
      mcpEnabled: 'Enabled',
      mcpApiKey: 'API Key',
      mcpMaxConcurrency: 'Max Concurrency',
      aiSettings: 'AI Settings',
      aiEnabled: 'Analysis Enabled',
      aiProviderId: 'Provider ID',
      aiModel: 'Model',
      aiTimeout: 'Timeout (seconds)',
      aiMinSeverity: 'Min Severity',
      aiNotificationGuard: 'AI Notification Guard',
      aiLanguage: 'AI Service Language',
      mediaRateLimitSettings: 'Media Rate Limit Settings',
      edit: 'Edit',
      save: 'Save',
      cancel: 'Cancel',
      reload: 'Reload',
      saveSuccess: 'Configuration saved successfully',
      reloadSuccess: 'Configuration reloaded successfully',
      persist: 'Save to Disk',
      persistSuccess: 'Configuration saved to disk',
      currentTheme: 'Current Theme',
      darkMode: 'Dark',
      lightMode: 'Light',
      switch: 'Switch',
      currentLocale: 'Current Language',
      sidebarCollapsed: 'Sidebar Collapsed',
      chatbarCollapsed: 'Chat Bar Collapsed'
    },
    profile: {
      title: 'Profile',
      username: 'Username',
      nickname: 'Nickname',
      email: 'Email',
      phone: 'Phone',
      qq: 'QQ Number',
      avatar: 'Avatar',
      emailInvalid: 'Please enter a valid email address',
      avatarHelp: 'PNG, JPG, GIF, or WebP. Max 5MB.',
      avatarTooLarge: 'Avatar file is too large (max 5MB)',
      avatarUploadSuccess: 'Avatar uploaded successfully',
      avatarUploadFailed: 'Failed to upload avatar',
      address: 'Address',
      introduction: 'Introduction',
      save: 'Save',
      reset: 'Reset',
      updated: 'Profile updated',
      created: 'Profile created',
      saveFailed: 'Save failed',
      loadFailed: 'Failed to load profile',
      roles: {
        superadmin: 'Super Admin',
        admin: 'Admin',
        user: 'User',
        unauthorized: 'Unauthorized'
      },
      greeting: "How's going on today?",
      main: 'Main'
    },
    alerts: {
      title: 'Alerts',
      alertLabel: 'Alert #{id}',
      add: 'Add Alert',
      search: 'Search alerts',
      filterSeverity: 'Filter severity',
      filterStatus: 'Filter status',
      filterAll: 'All',
      loading: 'Loading alerts...',
      noAlerts: 'No alerts found',
      noResults: 'No alerts match your filters',
      retry: 'Retry',
      consult: 'Consult AI',
      edit: 'Edit',
      remove: 'Delete',
      createdAt: 'Created At',
      messageLabel: 'Message',
      severityLabel: 'Severity',
      statusLabel: 'Status',
      commentLabel: 'Comment',
      addTitle: 'Add Alert',
      editTitle: 'Edit Alert',
      enterMessage: 'Enter alert message',
      enterComment: 'Add details or resolution...',
      selectSeverity: 'Select severity',
      selectStatus: 'Select status',
      cancel: 'Cancel',
      update: 'Update',
      create: 'Create',
      confirmDelete: 'Confirm Delete',
      confirmDeleteText: 'Are you sure you want to delete this alert?',
      aiTitle: 'AI Consultation',
      aiLoading: 'Consulting AI...',
      aiResponse: 'AI Response',
      adoptTitle: 'Adopt AI Suggestions',
      aiCommentPlaceholder: 'Summary of AI analysis...',
      close: 'Close',
      severityDisaster: 'Disaster',
      severityCritical: 'Critical',
      severityHigh: 'High',
      severityAverage: 'Average',
      severityWarning: 'Warning',
      severityInfo: 'Information',
      severityMedium: 'Medium',
      severityLow: 'Low',
      severityNotClassified: 'Not Classified',
      statusOpen: 'Open',
      statusAcknowledged: 'Acknowledged',
      statusResolved: 'Resolved',
      statusClosed: 'Closed',
      validationMessage: 'Please enter a message',
      validationSeverity: 'Please select severity',
      validationStatus: 'Please select status'
    },
    hosts: {
      title: 'End-point Assets',
      create: 'Create a new Asset',
      createTitle: 'Create a new Asset',
      search: 'Search assets',
      filterStatus: 'Filter status',
      filterMonitor: 'Filter monitor',
      syncMonitor: 'Sync monitor',
      syncHosts: 'Sync Hosts',
      syncAll: 'Sync All',
      syncItems: 'Sync Items',
      filterAll: 'All',
      name: 'Name',
      ip: 'IP Address',
      hostId: 'Host ID',
      description: 'Description',
      status: 'Status',
      lastSync: 'Last Sync',
      externalSource: 'External Source',
      cancel: 'Cancel',
      createBtn: 'Create',
      loading: 'Loading hosts...',
      noHosts: 'No hosts found',
      noResults: 'No hosts match your filters',
      retry: 'Retry',
      monitor: 'Monitor',
      actions: 'Actions',
      up: 'Up',
      down: 'Down',
      ai: 'AI',
      details: 'Details',
      detailTitle: 'Host Details',
      itemStatusChart: 'Item Status Overview',
      activeItemsTrend: 'Active Items Trend',
      compareTrends: 'Compare',
      statusLegendTitle: 'Status Legend',
      importantOnly: 'Important only',
      itemList: 'Items',
      items: 'Items',
      properties: 'Properties',
      delete: 'Delete',
      propertiesTitle: 'Properties',
      save: 'Save',
      saveAndPush: 'Save and Push',
      confirm: 'Confirm',
      deleteConfirmTitle: 'Warning',
      deleteConfirmText: 'Are you sure you want to delete {name}?',
      validationName: 'Please enter host name',
      created: 'Host created successfully!',
      createFailed: 'Failed to create host',
      updated: 'Host updated successfully!',
      updateFailed: 'Failed to update host',
      asyncPushStarted: 'Operation started. Monitor synchronization is running in the background.',
      deleted: 'Host deleted successfully!',
      deleteFailed: 'Failed to delete host',
      deleteCanceled: 'Delete canceled',
      syncHostsSuccess: 'Hosts synchronized',
      syncHostsFailed: 'Failed to sync hosts',
      syncAllSuccess: 'Hosts and items synchronized',
      syncAllFailed: 'Failed to sync hosts and items',
      syncItemsSuccess: 'Items synchronized for {name}',
      syncItemsFailed: 'Failed to sync items for {name}',
      pullHosts: 'Pull Hosts',
      pullHostsSuccess: 'Hosts pulled successfully',
      pullHostsFailed: 'Failed to pull hosts',
      pushHosts: 'Push Hosts',
      pushHostsSuccess: 'Hosts pushed successfully',
      pushHostsFailed: 'Failed to push hosts',
      pullItems: 'Pull Items',
      pullItemsSuccess: 'Items pulled for {name}',
      pullItemsFailed: 'Failed to pull items for {name}',
      pushItems: 'Push Items',
      pushItemsSuccess: 'Items pushed for {name}',
      pushItemsFailed: 'Failed to push items for {name}',
      terminal: 'WebSSH Terminal',
      sshTitle: 'SSH Configuration',
      sshUser: 'SSH User',
      sshPassword: 'SSH Password',
      sshPort: 'SSH Port',
      passwordPlaceholder: 'Leave empty to keep current password',
      aiError: 'Error',
      aiFailed: 'Failed to consult AI',
      unknownError: 'Unknown error',
      aiTitle: 'AI Consultation',
      aiLoading: 'Consulting AI...',
      aiResponse: 'AI Response',
      close: 'Close',
      unknown: 'Unknown',
      cpuUsage: 'CPU Usage',
      memoryUsage: 'Memory Usage',
      networkTraffic: 'Network Traffic',
      reportGenerate: 'Generate Report',
      reportTitle: 'Host Report',
      reportSummary: 'Summary',
      reportKeyMetrics: 'Key Metrics',
      reportChatTitle: 'Chat Highlights',
      reportChatEmpty: 'No chat highlights available.',
      reportDisclaimer: 'This report is auto-generated based on current host metrics and AI analysis. For critical decisions, please verify with additional tools.',
      reportSuccess: 'Report generated successfully!',
      reportFailed: 'Failed to generate report',
      networkInfo: 'Network Information',
      location: 'Location',
      geolocationLoading: 'Loading geolocation...',
      geolocationFailed: 'Failed to fetch location'
    },
    monitors: {
      title: 'Inventory Sources',
      create: 'Create a new Source',
      createTitle: 'Create a new Source',
      search: 'Search sources',
      searchField: 'Search Field',
      searchAll: 'All Fields',
      filterStatus: 'Filter status',
      filterType: 'Filter type',
      filterAll: 'All',
      statusOnline: 'Online',
      statusOffline: 'Offline',
      name: 'Name',
      url: 'URL',
      username: 'Username',
      password: 'Password',
      type: 'Type',
      description: 'Description',
      noDescription: 'No description provided',
      status: 'Status',
      cancel: 'Cancel',
      createBtn: 'Create',
      loading: 'Loading monitors...',
      retry: 'Retry',
      noMonitors: 'No monitors found',
      noResults: 'No monitors match your filters',
      properties: 'Properties',
      delete: 'Delete',
      login: 'Login',
      reLogin: 'Re-login',
      syncGroups: 'Sync Groups',
      propertiesTitle: 'Properties',
      save: 'Save',
      unknown: 'Unknown'
    },
    alarms: {
      title: 'Alert Sources',
      create: 'Create a new Source',
      createTitle: 'Create a new Source',
      search: 'Search sources',
      searchField: 'Search Field',
      searchAll: 'All Fields',
      filterStatus: 'Filter status',
      filterAll: 'All',
      name: 'Name',
      url: 'URL',
      username: 'Username',
      password: 'Password',
      type: 'Type',
      description: 'Description',
      noDescription: 'No description provided',
      status: 'Status',
      cancel: 'Cancel',
      createBtn: 'Create',
      loading: 'Loading alarms...',
      retry: 'Retry',
      noAlarms: 'No alarms found',
      noResults: 'No alarms match your filters',
      properties: 'Properties',
      delete: 'Delete',
      login: 'Login',
      reLogin: 'Re-login',
      propertiesTitle: 'Properties',
      save: 'Save',
      initializeZabbix: 'Initialize Zabbix',
      initializeZabbixTooltip: 'Sync/Update media types and actions in Zabbix',
      unknown: 'Unknown'
    },
    items: {
      itemStatusChart: 'Metric Status Overview',
      importantOnly: 'Important Only',
      total: 'Total Items',
      details: 'Details',
      detailTitle: 'Item Details',
      statusChart: 'Status Chart',
      host: 'Host ID',
      comment: 'Comment',
      titleAll: 'Metric Explorer',
      titleHost: 'Metrics for Asset #{id}',
      titleHostName: 'Metrics for {name}',
      add: 'Add Metric',
      search: 'Search metrics',
      filterHost: 'Filter host',
      filterStatus: 'Filter status',
      filterType: 'Filter type',
      filterAll: 'All',
      loading: 'Loading items...',
      retry: 'Retry',
      noItems: 'No monitoring items found',
      noResults: 'No items match your filters',
      id: 'ID',
      name: 'Name',
      value: 'Value',
      units: 'Units',
      type: 'Type',
      interval: 'Interval',
      status: 'Status',
      enabled: 'Enabled',
      disabled: 'Disabled',
      description: 'Description',
      actions: 'Actions',
      ai: 'AI',
      edit: 'Edit',
      delete: 'Delete',
      addTitle: 'Add Item',
      editTitle: 'Edit Item',
      enterName: 'Enter item name',
      enterValue: 'Enter item value (e.g., cpu.usage)',
      selectType: 'Select type',
      intervalSeconds: 'Interval in seconds',
      statusEnabled: 'Enabled',
      statusDisabled: 'Disabled',
      enterDescription: 'Enter description',
      cancel: 'Cancel',
      update: 'Update',
      create: 'Create',
      validationName: 'Please enter a name',
      loadFailed: 'Failed to load items',
      created: 'Item created successfully',
      updated: 'Item updated successfully',
      saveFailed: 'Failed to save item',
      deleted: 'Item deleted successfully',
      deleteFailed: 'Failed to delete item',
      sync: 'Sync Items',
      syncSuccess: 'Items synchronized',
      syncFailed: 'Failed to sync items',
      syncSelectHost: 'Please select a host first',
      pull: 'Pull Items',
      pullSuccess: 'Items pulled successfully',
      pullFailed: 'Failed to pull items',
      pullSelectHost: 'Please select a host first',
      push: 'Push Items',
      saveAndPush: 'Save and Push',
      createAndPush: 'Create and Push',
      pushSuccess: 'Items pushed successfully',
      pushFailed: 'Failed to push items',
      pushSelectHost: 'Please select a host first',
      selectedCount: 'Selected: {count}',
      bulkUpdate: 'Bulk Update',
      bulkDelete: 'Bulk Delete',
      bulkUpdateTitle: 'Bulk Update Items',
      bulkUpdateEnabled: 'Enabled',
      bulkUpdateStatus: 'Status',
      bulkUpdateNoChange: 'No change',
      bulkUpdateNoChanges: 'Please choose at least one field to update',
      bulkUpdateSuccess: 'Updated {count} items',
      bulkUpdateFailed: 'Failed to update items',
      bulkDeleteConfirmTitle: 'Confirm Bulk Delete',
      bulkDeleteConfirmText: 'Delete {count} selected items?',
      bulkDeleteSuccess: 'Deleted {count} items',
      bulkDeleteFailed: 'Failed to delete selected items',
      selectAtLeastOne: 'Please select at least one item',
      sort: 'Sort by',
      sortNameAsc: 'Name (A-Z)',
      sortNameDesc: 'Name (Z-A)',
      sortCreatedAsc: 'Oldest created',
      sortCreatedDesc: 'Newest created',
      sortUpdatedAsc: 'Oldest updated',
      sortUpdatedDesc: 'Recently updated',
      sortStatusDesc: 'Status (High-Low)',
      sortStatusAsc: 'Status (Low-High)',
      apply: 'Apply',
      aiError: 'Error',
      aiFailed: 'Failed to consult AI',
      unknownError: 'Unknown error',
      confirmDelete: 'Confirm Delete',
      confirmDeleteText: 'Are you sure you want to delete this item?',
      aiTitle: 'AI Consultation',
      aiLoading: 'Consulting AI...',
      aiResponse: 'AI Response',
      close: 'Close'
    },
    packets: {
      title: 'Packet Analysis',
      subtitle: 'Analyze captured network traffic using AI to detect anomalies or malicious flows.',
      upload: 'Upload Capture',
      uploadTitle: 'Analyze New Packet/Flow',
      name: 'Session Name',
      provider: 'AI Provider',
      model: 'Model',
      file: 'PCAP/Text File',
      rawContent: 'Raw Hex/Flow Data (Optional)',
      rawContentPlaceholder: 'Paste hex or flow data here...',
      analyze: 'Start Analysis',
      status: 'Status',
      risk: 'Risk Level',
      actions: 'Actions',
      reAnalyze: 'Re-analyze',
      delete: 'Delete',
      noItems: 'No packet analyses yet.',
      analyzing: 'Analyzing...',
      completed: 'Completed',
      failed: 'Failed',
      pending: 'Pending',
      clean: 'Clean',
      notable: 'Notable',
      malicious: 'Malicious',
      summary: 'Analysis Summary',
      result: 'Result',
      close: 'Close',
      success: 'Packet uploaded and analysis started.',
      deleted: 'Analysis deleted successfully.',
      namePlaceholder: 'e.g. Suspicious TCP Flow',
      providerLabel: 'Provider',
      sourceFileLabel: 'Source File',
      manualContent: 'Manual Content',
      rawDataTitle: 'Raw Data / Flow',
      aiAnalysisTitle: 'AI Intelligence Analysis',
      loadFailed: 'Failed to load data',
      uploadFailed: 'Upload failed',
      analysisStarted: 'Analysis started',
      startFailed: 'Failed to start analysis',
      deleteFailed: 'Delete failed',
      unknown: 'Unknown',
      nameRequired: 'Please enter a name',
      providerRequired: 'Please select an AI provider',
      dropFile: 'Drop file here or click to upload'
    },
    providers: {
      title: 'AI & Services',
      create: 'Create a new Provider',
      search: 'Search providers',
      filterStatus: 'Filter status',
      filterAll: 'All',
      createTitle: 'Create Provider',
      editTitle: 'Edit Provider',
      loading: 'Loading providers...',
      retry: 'Retry',
      noProviders: 'No providers found',
      noResults: 'No providers match your filters',
      statusOperational: 'Operational',
      statusOffline: 'Offline',
      properties: 'Properties',
      delete: 'Delete',
      type: 'Type',
      providerName: 'Provider Name',
      url: 'URL',
      apiKey: 'API Key',
      defaultModel: 'Default Model',
      description: 'Description',
      status: 'Status',
      cancel: 'Cancel',
      edit: 'Edit',
      createBtn: 'Create',
      propertiesTitle: 'Properties',
      name: 'Name',
      model: 'Model',
      fetchModels: 'Fetch Models',
      save: 'Save',
      nameRequired: 'Provider name is required',
      apiKeyRequired: 'API key is required',
      typeRequired: 'Provider type is required',
      typeGemini: 'Gemini',
      typeOpenAI: 'OpenAI',
      typeOther: 'Other (OpenAI-compatible)',
      checkStatus: 'Check Status',
      checkAll: 'Check All',
    },
    users: {
      title: 'Identity Management',
      search: 'Search users',
      create: 'Create User',
      loading: 'Loading users...',
      retry: 'Retry',
      noUsers: 'No users found',
      id: 'ID',
      username: 'Username',
      nickname: 'Nickname',
      email: 'Email',
      phone: 'Phone',
      qq: 'QQ',
      role: 'Role',
      status: 'Status',
      active: 'Active',
      inactive: 'Inactive',
      actions: 'Actions',
      edit: 'Edit',
      delete: 'Delete',
      createTitle: 'Create User',
      editTitle: 'Edit User',
      password: 'Password',
      passwordHint: 'Leave blank to keep unchanged',
      avatar: 'Avatar',
      address: 'Address',
      introduction: 'Introduction',
      cancel: 'Cancel',
      update: 'Update',
      createBtn: 'Create',
      confirmDelete: 'Confirm Delete',
      confirmDeleteText: 'Are you sure you want to delete this user?',
      usernameRequired: 'Username is required',
      passwordRequired: 'Password is required',
      created: 'User created',
      updated: 'User updated',
      deleted: 'User deleted',
      loadFailed: 'Failed to load users',
      loadInfoFailed: 'Failed to load user information',
      operationFailed: 'Operation failed',
      deleteFailed: 'Delete failed',
      roleUnauthorized: 'Unauthorized',
      roleUser: 'User',
      roleAdmin: 'Administrator',
      roleSuper: 'Super Administrator'
    },
    menu: {
      databoard: 'Databoard',
      insights: 'Insights',
      systemStatus: 'System Health',
      metricRacing: 'Metric Racing',
      analytics: 'Trend Analytics',
      reports: 'Operational Reports',
      inventory: 'Asset Management',
      group: 'Host Groups',
      host: 'End-point Assets',
      observability: 'Observability',
      item: 'Metric Explorer',
      alerting: 'Incident Response',
      alert: 'Current Alerts',
      automation: 'Alert Logic',
      trigger: 'Trigger Rules',
      action: 'Execution Actions',
      sources: 'Data Ingestion',
      monitor: 'Inventory Sources',
      alarm: 'Alert Sources',
      tooling: 'Remediation Tools',
      terminal: 'Remote Terminal',
      kb: 'Knowledge Base',
      communication: 'Communication Channels',
      media: 'Notification Targets',
      provider: 'AI & Services',
      providers: 'AI Providers',
      system: 'Platform Settings',
      user: 'Identity Management',
      registerApplication: 'On-boarding Requests',
      log: 'System Logs',
      auditLog: 'Audit Logs',
      configuration: 'System Config',
      profile: 'User Profile',
      about: 'About',
      maintenance: 'Maintenance',
      delivery: 'Delivery',
      users: 'Users',
    },
    media: {
      title: 'Media',
      create: 'Create Media',
      createTitle: 'Create Media',
      search: 'Search media',
      filterStatus: 'Filter status',
      filterAll: 'All',
      name: 'Name',
      type: 'Type',
      typeHint: 'email/other/qq',
      target: 'Target',
      targetHint: 'address or endpoint',
      params: 'Params',
      paramsHint: 'JSON parameters for template',
      description: 'Description',
      status: 'Status',
      actions: 'Actions',
      test: 'Test',
      testSuccess: 'Test message sent successfully',
      testFailed: 'Test failed',
      properties: 'Properties',
      save: 'Save',
      cancel: 'Cancel',
      delete: 'Delete',
      loading: 'Loading media...',
      loadFailed: 'Failed to load media',
      noMedia: 'No media found',
      noResults: 'No media match your filters',
      retry: 'Retry',
      created: 'Media created successfully!',
      updated: 'Media updated successfully!',
      deleted: 'Media deleted successfully!',
      createFailed: 'Failed to create media',
      updateFailed: 'Failed to update media',
      deleteFailed: 'Failed to delete media',
      deleteCanceled: 'Delete canceled',
      validationName: 'Please enter media name',
      validationType: 'Please select media type',
      unknownType: 'Unknown type'
    },
    actions: {
      title: 'Actions',
      create: 'Create Action',
      createTitle: 'Create Action',
      search: 'Search actions',
      filterStatus: 'Filter status',
      filterAll: 'All',
      name: 'Name',
      severity: 'Severity',
      severityMin: 'Min Severity',
      alertStatus: 'Alert Status',
      host: 'Host',
      group: 'Group',
      trigger: 'Trigger',
      media: 'Media',
      template: 'Template',
      templateHint: 'Use {{alert_id}} {{message}} {{severity}} {{status}} {{host_id}} {{host_name}} {{item_id}} {{name}} {{value}} {{units}} {{monitor_id}} {{group_id}} {{entity}} {{added}} {{updated}} {{failed}} {{total}}',
      description: 'Description',
      status: 'Status',
      actions: 'Actions',
      properties: 'Properties',
      save: 'Save',
      cancel: 'Cancel',
      delete: 'Delete',
      loading: 'Loading actions...',
      loadFailed: 'Failed to load actions',
      noActions: 'No actions found',
      noResults: 'No actions match your filters',
      retry: 'Retry',
      created: 'Action created successfully!',
      updated: 'Action updated successfully!',
      deleted: 'Action deleted successfully!',
      createFailed: 'Failed to create action',
      updateFailed: 'Failed to update action',
      deleteFailed: 'Failed to delete action',
      deleteCanceled: 'Delete canceled',
      validationName: 'Please enter action name',
      unknownMedia: 'Unknown media',
      test: 'Test',
      testSuccess: 'Test action requested successfully',
      testFailed: 'Test action failed'
    },
    triggers: {
      title: 'Triggers',
      create: 'Create Trigger',
      createTitle: 'Create Trigger',
      search: 'Search triggers',
      filterStatus: 'Filter status',
      filterAll: 'All',
      name: 'Name',
      entity: 'Entity',
      entityAlert: 'Alert',
      entityLog: 'Log',
      entityItem: 'Item',
      severity: 'Severity',
      severityMin: 'Min Severity',
      alertStatus: 'Alert Status',
      alertQuery: 'Alert Query',
      alertId: 'Alert ID',
      alertMonitorId: 'Monitor ID',
      alertGroupId: 'Group ID',
      alertHostId: 'Host ID',
      alertItemId: 'Item ID',
      hostName: 'Host',
      itemName: 'Item',
      logType: 'Log Type',
      logSeverity: 'Log Severity',
      logQuery: 'Log Query',
      itemStatus: 'Item Status',
      itemHostId: 'Host Name',
      itemId: 'Item Name',
      itemValueOperator: 'Value Operator',
      itemValueOperatorHint: 'Select operator',
      itemValueThreshold: 'Value Threshold',
      monitorField: 'Monitor Field',
      monitorFieldStatus: 'Item Status',
      monitorFieldValue: 'Item Value',
      operatorBetween: 'Between',
      operatorOutside: 'Outside',
      action: 'Action',
      status: 'Status',
      actions: 'Actions',
      properties: 'Properties',
      save: 'Save',
      cancel: 'Cancel',
      delete: 'Delete',
      loading: 'Loading triggers...',
      loadFailed: 'Failed to load triggers',
      noTriggers: 'No triggers found',
      noResults: 'No triggers match your filters',
      retry: 'Retry',
      created: 'Trigger created successfully!',
      updated: 'Trigger updated successfully!',
      deleted: 'Trigger deleted successfully!',
      createFailed: 'Failed to create trigger',
      updateFailed: 'Failed to update trigger',
      deleteFailed: 'Failed to delete trigger',
      deleteCanceled: 'Delete canceled',
      validationName: 'Please enter trigger name',
      validationAction: 'Please select an action',
      unknownAction: 'Unknown action'
    },
    logs: {
      title: 'Logs',
      systemTitle: 'System Logs',
      serviceTitle: 'Service Logs',
      search: 'Search logs',
      filterLevel: 'Filter level',
      filterAll: 'All',
      level: 'Level',
      levelInfo: 'Information',
      levelWarn: 'Warning',
      levelAverage: 'Average',
      levelError: 'Error',
      levelCritical: 'Critical',
      levelFatal: 'Fatal',
      message: 'Message',
      context: 'Context',
      ip: 'IP',
      createdAt: 'Created At',
      refresh: 'Refresh',
      loading: 'Loading logs...',
      retry: 'Retry',
      noLogs: 'No logs found',
      loadFailed: 'Failed to load logs',
      details: 'Log Details',
      clear: 'Clear Logs',
      export: 'Export CSV',
      autoRefresh: 'Auto Refresh',
      rawContext: 'Raw Context',
      formattedContext: 'Formatted Context',
      retry: 'Retry',
      applyAndSave: 'Apply and Save'
    },
    groups: {
      title: 'Host Groups',
      create: 'Create a new Group',
      createTitle: 'Create a new Group',
      search: 'Search groups',
      filterStatus: 'Filter status',
      filterAll: 'All',
      name: 'Name',
      description: 'Description',
      status: 'Status',
      enabled: 'Enabled',
      hosts: 'Hosts',
      items: 'Items',
      actions: 'Actions',
      details: 'Details',
      properties: 'Properties',
      save: 'Save',
      cancel: 'Cancel',
      delete: 'Delete',
      loading: 'Loading groups...',
      loadFailed: 'Failed to load groups',
      noGroups: 'No groups found',
      noResults: 'No groups match your filters',
      retry: 'Retry',
      created: 'Group created successfully!',
      updated: 'Group updated successfully!',
      deleted: 'Group deleted successfully!',
      createFailed: 'Failed to create group',
      updateFailed: 'Failed to update group',
      deleteFailed: 'Failed to delete group',
      deleteCanceled: 'Delete canceled',
      pull: 'Pull Group',
      pullSuccess: 'Group pulled successfully',
      pullFailed: 'Failed to pull group',
      push: 'Push Group',
      pushSuccess: 'Group pushed successfully',
      pushFailed: 'Failed to push group',
      detailTitle: 'Group Details',
      summary: 'Summary',
      totalHosts: 'Total Hosts',
      activeHosts: 'Active Hosts',
      errorHosts: 'Error Hosts',
      syncingHosts: 'Syncing Hosts',
      totalItems: 'Total Items',
      selectGroup: 'Select a group to view details',
      hostStatusChart: 'Host Status Overview'
    },
    configuration: {
      title: 'System Configuration',
      subtitle: 'Manage system-wide settings and integrations',
      system: 'System Settings',
      database: 'Database Settings',
      services: 'Background Services',
      syncService: 'Sync Service',
      statusCheckService: 'Status Check Service',
      integrations: 'Integrations',
      aiAnalysis: 'AI Analysis',
      onebotSettings: 'OneBot (NapCat) Configuration',

      mcpSettings: 'MCP Settings',
      advanced: 'Advanced',
      siteMessageSettings: 'Site Message Settings',
      minAlertSeverity: 'Min Alert Severity',
      minLogSeverity: 'Min Log Severity',
      mediaRateLimitSettings: 'Media Rate Limit Settings',
      globalInterval: 'Global Interval (s)',
      protocolInterval: 'Protocol Interval (s)',
      mediaInterval: 'Media Interval (s)',
      onebotEnabled: 'OneBot Enabled',
      onebotMode: 'Connection Mode',
      onebotModeReverse: 'Reverse (NapCat as Client)',
      onebotModePositive: 'Positive (Server as Client)',
      onebotUrl: 'WebSocket URL',
      onebotToken: 'Access Token',

      external: 'External',
      mcpServers: 'MCP Servers',
      addMcpServer: 'Add MCP Server',
      mcpServerName: 'Server Name',
      mcpServerCommand: 'Command',
      mcpServerApiKey: 'API Key',
      mcpServerApiKeyPlaceholder: 'Optional for servers requiring authentication',
      mcpServerApiKeyEnv: 'API Key Env',
      mcpServerArgs: 'Arguments (one per line)',
      mcpServerArgsPlaceholder: '-y\n@modelcontextprotocol/server-filesystem\nD:/Nagare_Project',
      mcpServerEnabled: 'Enabled',
      mcpServerTest: 'Test',
      mcpServerHelpText: 'Each server is started as a local MCP stdio process. Save changes to reconnect clients with the new settings.',
      testConnection: 'Test',
      mcpTestSuccess: 'Connection test successful, tools loaded',
      mcpTestFailed: 'Connection test failed',
      mcpClientStatus: 'MCP Client Status',
      refreshMcpStatus: 'Refresh Status',
      mcpStatusTotal: 'Configured',
      mcpStatusConnected: 'Connected',
      mcpServerState: 'State',
      mcpStateConnected: 'Connected',
      mcpStateDisconnected: 'Disconnected',
      mcpServerToolCount: 'Tools',
      mcpServerUpdatedAt: 'Updated At',
      mcpServerLastError: 'Last Error',
      mcpStatusLoadFailed: 'Failed to load MCP client status',
      unknownError: 'Unknown error',
      externalInfrastructure: 'External Infrastructure',
      addItem: 'Add Definition',
      itemType: 'Type',
      itemKey: 'Key',
      itemName: 'Name',
      itemId: 'ID',
      itemActions: 'Actions',
      itemHelpText: 'These definitions define the available types for monitors, alarms, AI providers, and notification media.',
      systemName: 'System Name',
      ipAddress: 'IP Address',
      port: 'Port',
      availability: 'Availability',
      dbVersion: 'Database Version',
      dbHost: 'Database Host',
      dbPort: 'Database Port',
      dbUsername: 'Database Username',
      dbPassword: 'Database Password',
      dbName: 'Database Name',
      save: 'Save Configuration',
      reload: 'Reload Configuration',
      reset: 'Reset Defaults',
      cancel: 'Cancel',
      edit: 'Edit',
      saving: 'Saving...',
      loading: 'Loading configuration...',
      saveSuccess: 'Configuration saved successfully!',
      saveFailed: 'Failed to save configuration',
      reloadSuccess: 'Configuration reloaded successfully!',
      reloadFailed: 'Failed to reload configuration',
      confirmSave: 'Save Changes?',
      confirmSaveText: 'Are you sure you want to save these configuration changes?',
      resetConfirmTitle: 'Reset to Defaults?',
      resetConfirmText: 'This will reset ALL configuration settings to their default system values. This action cannot be undone. Are you sure?',
      resetConfirmYes: 'Yes, Reset Everything',
      resetConfirmNo: 'No, Keep My Settings',
      resetSuccess: 'Configuration has been reset to defaults.',
      yes: 'Yes',
      no: 'No'
    },
    kb: {
      title: 'Knowledge Base',
      subtitle: 'Manage local knowledge for RAG alert analysis',
      searchPlaceholder: 'Search topics, content or keywords...',
      create: 'Add Knowledge',
      createTitle: 'Add Knowledge Base Entry',
      editTitle: 'Edit Knowledge Base Entry',
      topic: 'Topic',
      category: 'Category',
      keywords: 'Keywords',
      content: 'Content',
      noItems: 'No knowledge base entries found',
      loadFailed: 'Failed to load knowledge base',
      deleteConfirm: 'Are you sure you want to delete this knowledge entry?',
      deleteSuccess: 'Entry deleted',
      deleteFailed: 'Failed to delete entry',
      createSuccess: 'Entry created',
      updateSuccess: 'Entry updated',
      saveFailed: 'Failed to save entry',
      topicRequired: 'Topic is required',
      contentRequired: 'Content is required',
      categoryRequired: 'Category is required'
    },
    reports: {
      title: 'Reports',
      subtitle: 'Automated Operations Reports',
      filterType: 'Filter Type',
      typeDaily: 'Daily',
      typeWeekly: 'Weekly',
      typeMonthly: 'Monthly',
      typeCustom: 'Custom',
      config: 'Configuration',
      configTitle: 'Report Configuration',
      generateDaily: 'Generate Daily',
      generateWeekly: 'Generate Weekly',
      generateMonthly: 'Generate Monthly',
      generateCustom: 'Generate Custom',
      customTitle: 'Generate Custom Report',
      titleLabel: 'Title',
      rangeLabel: 'Range',
      autoGenerate: 'Auto Generate',
      generateDay: 'Generate Day',
      generateTime: 'Generate Time',
      generateDate: 'Generate Date',
      reportTitle: 'Title',
      type: 'Type',
      generatedAt: 'Generated At',
      status: 'Status',
      loadFailed: 'Failed to load reports',
      generationStarted: 'Report generation started',
      generationFailed: 'Report generation failed',
      deleteConfirm: 'Are you sure you want to delete this report?',
      deleteSuccess: 'Report deleted successfully',
      language: 'Language',
      customDefaultTitle: 'Custom Infrastructure Report',
      enableAISummary: 'AI Summary',
    },
    message: {
      title: 'Notifications',
      empty: 'No new notifications',
      markAllRead: 'Mark all as read',
      markAllReadSuccess: 'All notifications marked as read',
      viewAll: 'View all history',
      historyTitle: 'Notification History',
      historySubtitle: 'View and manage your system notifications',
      filterAll: 'All',
      filterUnread: 'Unread',
      colTitle: 'Title',
      colContent: 'Content',
      colType: 'Type',
      colTime: 'Time',
      loadFailed: 'Failed to load notifications',
      deleteConfirm: 'Delete this notification?',
      deleteSuccess: 'Notification deleted',
      deleteFailed: 'Failed to delete notification',
      operationFailed: 'Operation failed'
    },
    terminal: {
      title: 'Terminal',
      selectHost: 'Select a host',
      switchHost: 'Switch Host',
      disconnect: 'Disconnect',
      reconnect: 'Reconnect',
      connectTitle: 'Connect to Host',
      savedHost: 'Saved Host',
      directConnect: 'Direct Connect',
      selectHostPlaceholder: 'Select a host',
      connect: 'Connect',
      noCredentials: 'SSH credentials not configured for this host. Please update host properties first.',
      fetchInfoFailed: 'Failed to fetch host information',
      connected: 'Connected to host',
      closed: 'Connection closed',
      wsError: 'WebSocket error',
      connFailed: 'WebSocket connection failed. Ensure the host is reachable and SSH credentials are correct.'
    },
    auditLog: {
      title: 'Security Audit Log',
      search: 'Search user actions...'
    },
    analytics: {
      title: 'Trend Analysis',
      subtitle: 'Insights and statistical trends from your monitoring data',
      chaosTitle: 'Chaos Simulator',
      chaosDesc: 'Trigger a simulated alert storm to test system resilience.',
      triggerChaos: 'Trigger Alert Storm',
      alertTrend: 'Alert Trend (Last 14 Days)',
      severityDist: 'Severity Distribution',
      topKeywords: 'Top Alert Keywords',
      alertIntensity: 'Alert Intensity (Heatmap)',
      topNoisyHosts: 'Top Noisy Hosts',
      alerts: 'Alerts',
      totalAlerts: 'Total Alerts',
      systemHealth: 'System Health',
      activeHosts: 'Active Hosts'
    },
    metricRacing: {
      title: 'Metric Racing',
      subtitle: 'Real-time performance ranking of infrastructure assets',
      cpuTitle: 'CPU Utilization (%)',
      netTitle: 'Network Traffic',
      itemTitle: 'Item Value',
      start: 'Start Racing',
      stop: 'Stop Racing'
    },
    retention: {
      title: 'Data Retention',
      subtitle: 'Manage how long system data is kept',
      dataType: 'Data Type',
      retentionDays: 'Retention (Days)',
      enabled: 'Enabled',
      description: 'Description',
      actions: 'Actions',
      edit: 'Edit',
      save: 'Save',
      cancel: 'Cancel',
      manualCleanup: 'Manual Cleanup',
      nextCleanup: 'Next cleanup: Today 02:00 AM',
      performCleanup: 'Perform Cleanup',
      cleanupConfirm: 'Manual Cleanup',
      cleanupConfirmText: 'Are you sure you want to perform manual data cleanup now according to the enabled policies?',
      cleanupSuccess: 'Cleanup Success',
      cleanupSuccessMsg: 'Cleanup performed successfully.',
      recordsRemoved: 'Records removed:',
      noRecordsCleaned: 'No records needed cleaning.',
      cleanupFailed: 'Cleanup failed',
      cleanupFailedMsg: 'Cleanup operation failed.',
      autoRefreshing: 'Auto-refreshing (30s)',
      enterDescription: 'Enter description',
      updateSuccess: 'Policy updated successfully',
      updateFailed: 'Failed to update policy',
      forever: 'Forever',
      automatedCleanupTitle: 'Automated Cleanup',
      automatedCleanupDesc: 'The system automatically performs data retention cleanup every day at 2:00 AM based on these policies.',
      types: {
        logs: 'System Logs',
        alerts: 'Alerts',
        audit_logs: 'Audit Logs',
        item_history: 'Item History',
        host_history: 'Host History',
        network_history: 'Network Health History',
        chat: 'AI Chat History',
        reports: 'Generated Reports',
        site_messages: 'Site Messages',
        ansible_jobs: 'Ansible Jobs',
        group_history: 'Group History',
        monitor_history: 'Monitor History'
      },
      typeDescriptions: {
        logs: 'System and service runtime logs.',
        alerts: 'History of system alerts and incidents.',
        audit_logs: 'Audit logs of user operational actions.',
        item_history: 'Time-series historical data for metrics.',
        host_history: 'Historical status and availability of hosts.',
        network_history: 'History of global network health scores.',
        chat: 'Conversation history with the AI assistant.',
        reports: 'Archive of generated PDF operational reports.',
        site_messages: 'Internal notifications and messages for users.',
        ansible_jobs: 'History of executed Ansible playbooks and jobs.',
        group_history: 'Historical status and availability of host groups.',
        monitor_history: 'Historical status and availability of monitoring systems.'
      }
    },
    about: {
      title: 'About Nagare',
      subtitle: 'Open-source infrastructure intelligence platform',
      description: 'Nagare is a deep-dive infrastructure intelligence and automated operations platform designed to provide comprehensive visibility and AI-assisted troubleshooting for modern environments.',
      disclaimer: 'As Nagare is still in active development, some features and overall stability may not yet be fully optimized. We sincerely appreciate your support in using and sharing this project, and we apologize for any inconvenience or technical shortcomings you may encounter during this early stage.',
      contact: 'If you encounter any issues, please feel free to reach out through the channels below:',
      github: 'GitHub Repository',
      author: 'Author',
      email: 'Email',
      version: 'Version',
      license: 'License'
    }
  },
  zh: {
    mcp: {
      title: 'MCP 服务',
      alertInfo: '配置将保存在本地文件 configs/mcp_config.json 中。修改后请点击“保存”以持久化生效。',
      serviceName: '服务名称',
      command: '执行命令',
      addTitle: '添加 MCP 服务',
      editTitle: '编辑 MCP 服务',
      name: '名称',
      args: '运行参数',
      argsPlaceholder: '输入后回车添加参数',
      envVars: '环境变量',
      addEnv: '添加变量',
      envKey: '键',
      envValue: '值',
      removedLocally: '本地列表中已移除。请点击顶部的“保存”按钮以持久化更改。',
      savedToList: '已更新至列表，请点击顶部的“保存”按钮以持久化保存。',
      saveSuccess: 'MCP 配置保存成功',
      deleteTitle: '操作确认',
      deleteConfirmText: '确定要移除此 MCP 服务吗？',
      testPassed: '测试通过！已发现可用工具: {count} 个',
      testFailed: '测试连接失败: {message}'
    },
    common: {
      language: '语言',
      day: '日间',
      night: '夜间',
      back: '返回',
      refresh: '刷新',
      loading: '正在加载...',
      startTime: '开始时间',
      endTime: '结束时间',
      noHistoryData: '暂无历史数据',
      historyLoadFailed: '加载历史失败',
      comparePrevious: '对比',
      currentPeriod: '当前',
      previousPeriod: '上一周期',
      expand: '展开',
      collapse: '收起',
      showChat: '显示聊天',
      hideChat: '隐藏聊天',
      search: '搜索',
      globalSearch: '全局搜索',
      searchPlaceholder: '搜索任何内容...',
      columns: '列',
      actions: '操作',
      add: '添加',
      test: '测试',
      status: '状态',
      confirm: '确定',
      enabled: '启用',
      disabled: '禁用',
      statusInactive: '未激活',
      statusActive: '活跃',
      statusError: '错误',
      statusSyncing: '同步中',
      reasonInactive: '未运行或被禁用',
      reasonActive: '运行正常',
      reasonError: '检测到错误',
      reasonSyncing: '正在同步',
      noMore: '没有更多数据',
      selectedCount: '已选 {count} 项',
      bulkUpdate: '批量更新',
      bulkDelete: '批量删除',
      bulkUpdateTitle: '批量更新',
      bulkUpdateNoChange: '不修改',
      bulkUpdateNoChanges: '请至少选择一个字段进行更新',
      bulkUpdateSuccess: '已更新 {count} 条记录',
      bulkUpdateFailed: '批量更新失败',
      bulkDeleteConfirmTitle: '确认批量删除',
      bulkDeleteConfirmText: '确定删除选中的 {count} 条记录吗？',
      bulkDeleteSuccess: '已删除 {count} 条记录',
      bulkDeleteFailed: '批量删除失败',
      selectAtLeastOne: '请至少选择一条记录',
      selectAll: '全选',
      deselectAll: '取消全选',
      sort: '排序',
      sortNameAsc: '名称 A-Z',
      sortNameDesc: '名称 Z-A',
      sortCreatedAsc: '创建时间最早',
      sortCreatedDesc: '创建时间最新',
      sortUpdatedAsc: '更新时间最早',
      sortUpdatedDesc: '最近更新',
      sortStatusDesc: '状态降序',
      sortStatusAsc: '状态升序',
      apply: '应用',
      footer: '页脚',
      ok: '确定',
      edit: '编辑',
      delete: '删除',
      save: '保存',
      saveLocally: '仅保存到本地',
      saveAndPush: '保存并推送',
      deleteLocally: '仅从本地删除',
      deleteAndPush: '删除并从外部同步',
      cancel: '取消',
      warning: '警告',
      invalidJson: 'JSON 格式无效',
      invalidEmail: '邮箱格式无效',
      weakPassword: '密码必须至少 8 个字符，且包含以下至少 3 种：小写字母、大写字母、数字、特殊符号',
      invalidUsername: '用户名必须是 3-32 个字符，仅支持字母、数字、下划线和连字符',
      invalidInput: '输入无效',
      unauthorizedTitle: '未授权',
      accessDeniedTitle: '访问被拒绝',
      operationFailed: '操作失败',
      sessionExpired: '会话已过期，请重新登录。',
      insufficientPrivileges: '权限不足，请使用授权账号登录。',
      loginToContinue: '请先登录。',
      dayUnit: '天',
      hourUnit: '小时',
      minuteUnit: '分',
      secondUnit: '秒',
      details: '日志详情',
      clear: '清空日志',
      export: '导出 CSV',
      autoRefresh: '自动刷新',
      rawContext: '原始上下文',
      formattedContext: '格式化上下文',
      retry: '重试',
      applyAndSave: '应用并保存',
      noData: '暂无数据',
      sync: '同步',
      view: '查看',
      unknown: '未知'
    },
    auth: {
      login: '登录',
      register: '注册',
      reset: '重置密码',
      logout: '退出登录',
      guest: '访客',
      signInTitle: '登录 Nagare',
      tagline: '深度基础设施洞察与自动化运维平台。',
      welcomeBack: '欢迎回来',
      rememberMe: '记住我',
      registerTitle: '提交注册申请',
      resetTitle: '重置密码',
      username: '用户名',
      email: '邮箱',
      password: '密码',
      confirmPassword: '确认密码',
      oldPassword: '旧密码',
      newPassword: '新密码',
      confirmNewPassword: '确认新密码',
      submit: '提交',
      backToLogin: '返回登录',
      createAccount: '创建账号',
      resetLink: '重置密码',
      enterCredentials: '请输入用户名和密码',
      emailInvalid: '请输入有效的邮箱地址',
      missingToken: '缺少令牌',
      loginFailed: '登录失败',
      passwordsMismatch: '两次密码不一致',
      passwordTooWeak: '密码强度太弱',
      passwordRules: '密码至少 8 位且包含大写、小写、数字、符号中的至少 3 种',
      applicationSubmitted: '申请已提交，请等待审核。',
      resetApplicationSubmitted: '重置申请已提交，请等待管理员审核。',
      registrationFailed: '注册失败',
      completeAllFields: '请填写所有字段',
      resetSuccess: '密码已重置，请登录。',
      resetFailed: '重置失败',
      submitResetRequest: '提交重置申请'
    },
    registerApplications: {
      title: '用户申请',
      search: '搜索申请',
      filterStatus: '筛选状态',
      filterAll: '全部',
      statusPending: '待审核',
      statusApproved: '已批准',
      statusRejected: '已拒绝',
      refresh: '刷新',
      loading: '正在加载申请...',
      retry: '重试',
      noApplications: '暂无申请',
      id: 'ID',
      username: '用户名',
      status: '状态',
      reason: '原因',
      approvedBy: '审批人',
      createdAt: '创建时间',
      actions: '操作',
      approve: '批准',
      reject: '拒绝',
      approved: '已批准申请',
      rejected: '已拒绝申请',
      approveSelected: '批量批准',
      rejectSelected: '批量拒绝',
      approveFailed: '批准失败',
      rejectFailed: '拒绝失败',
      rejectTitle: '拒绝申请',
      reasonPlaceholder: '请输入原因（可选）',
      cancel: '取消',
      loadFailed: '加载申请失败'
    },
    chat: {
      title: '聊天助手',
      subtitle: '可以咨询基础设施相关问题。',
      selectProvider: '选择供应商',
      section: '聊天',
      loadingHistory: '加载历史记录...',
      historyLoaded: '历史记录已加载',
      thinking: '思考中...',
      placeholder: '请输入问题...',
      send: '发送',
      empty: '开始对话后将在此显示消息。',
      selectProviderWarn: '请先选择供应商',
      toneLabel: '语气',
      toneProfessional: '专业',
      toneRoast: '毒舌',
      toolMode: '工具模式',
      toolBadge: '工具'
    },
    dashboard: {
      title: '系统面板',
      loading: '正在加载面板数据...',
      alerts: '告警',
      hosts: '主机',
      groups: '分组',
      monitors: '监控',
      providers: '供应商',
      critical: '严重',
      online: '在线',
      active: '活跃',
      recentAlerts: '最近告警',
      hostStatus: '主机状态',
      aiProviders: 'AI 供应商',
      viewAll: '查看全部',
      id: 'ID',
      message: '消息',
      severity: '严重级别',
      name: '名称',
      ip: 'IP',
      status: '状态',
      model: '模型',
      noAlerts: '暂无告警',
      noHosts: '暂无主机',
      noGroups: '暂无分组',
      noProviders: '暂无供应商',
      onlineLabel: '在线',
      offlineLabel: '离线',
      activeLabel: '活跃',
      inactiveLabel: '未启用',
      healthScore: '健康评分',
      healthGood: '健康',
      healthWarn: '降级',
      healthBad: '严重',
      activeMonitors: '活跃监控',
      activeGroups: '活跃分组',
      impactedGroups: '受影响分组',
      activeHosts: '活跃主机',
      activeItems: '活跃指标',
      networkMetrics: '网络指标',
      metricsSearch: '搜索指标',
      loadingMetrics: '正在加载指标...',
      noMetrics: '暂无指标',
      value: '数值',
      units: '单位',
      updated: '更新时间',
      refresh: '刷新',
      refreshAll: '全部刷新',
      host: '主机',
      metric: '指标',
      lastUpdated: '最近刷新',
      summaryLastUpdated: '面板已刷新',
      healthLoadFailed: '加载健康评分失败',
      metricsLoadFailed: '加载指标失败',
      healthTrendTitle: '网络健康趋势',
      topologyTitle: '网络拓扑',
      topologyMonitor: '资产来源',
      topologySite: '主机分组',
      topologyHost: '终端资产',
      impactedLabel: '受影响',
      loadingTopology: '正在加载拓扑...',
      noTopology: '暂无拓扑数据',
      topologyLoadFailed: '加载拓扑失败',
      voiceTitle: '语音运维指令',
      voiceStart: '开始语音',
      voiceStop: '停止语音',
      voiceListening: '聆听中',
      voiceIdle: '空闲',
      voiceHint: '试试 "查询健康" "查看告警" "查询交换机" "打开拓扑"',
      voiceTranscript: '识别内容',
      voiceAction: '动作',
      voiceActionHealth: '获取系统健康',
      voiceActionAlerts: '打开告警页面',
      voiceActionSwitch: '搜索交换机主机',
      voiceActionTopology: '聚焦拓扑图',
      voiceNoMatch: '未匹配到指令',
      voiceNotSupported: '浏览器不支持语音识别。',
      voiceFailed: '语音识别失败，请重试。',
      voiceHealthResult: '健康评分',
      voiceHealthFailed: '获取健康评分失败',
      matrixTitle: '矩阵日志流',
      matrixPause: '暂停',
      matrixResume: '继续'
    },
    systemStatus: {
      title: '系统健康',
      uptime: '运行时间',
      goroutines: '协程数',
      allocatedMem: '已分配内存',
      cpus: 'CPU核心数',
      memoryUsage: '内存使用 (MB)',
      systemInfo: '系统信息',
      goVersion: 'Go 版本',
      totalAlloc: '累计分配',
      systemMemory: '系统内存',
      gcCount: 'GC 次数'
    },
    system: {
      title: '配置',
      configuration: '配置',
      display: '显示',
      loading: '正在加载配置...',
      retry: '重试',
      systemSettings: '系统设置',
      systemName: '系统名称',
      ipAddress: 'IP 地址',
      availability: '可用性',
      port: '端口',
      databaseSettings: '数据库设置',
      dbVersion: '版本',
      dbHost: '主机',
      dbPort: '端口',
      dbUser: '用户名',
      dbPassword: '密码',
      dbName: '数据库名称',
      syncSettings: '同步设置',
      syncEnabled: '启用',
      syncInterval: '间隔（秒）',
      syncConcurrency: '并发数',
      statusCheckSettings: '状态检查设置',
      statusCheckEnabled: '启用',
      statusCheckInterval: '间隔（秒）',
      statusCheckProviderEnabled: '自动检查供应商',
      statusCheckConcurrency: '并发数',
      mcpSettings: 'MCP 设置',
      mcpEnabled: '启用',
      mcpApiKey: 'API Key',
      mcpMaxConcurrency: '最大并发',
      aiSettings: 'AI 设置',
      aiEnabled: '启用分析',
      aiProviderId: '供应商 ID',
      aiModel: '模型',
      aiTimeout: '超时（秒）',
      aiMinSeverity: '最小严重级别',
      aiNotificationGuard: 'AI 告警防护',
      aiLanguage: 'AI 服务语言',
      mediaRateLimitSettings: '通知频率限制设置',
      edit: '编辑',
      save: '保存',
      cancel: '取消',
      reload: '重新加载',
      saveSuccess: '配置保存成功',
      reloadSuccess: '配置重新加载成功',
      persist: '保存到磁盘',
      persistSuccess: '配置已保存到磁盘',
      currentTheme: '当前主题',
      darkMode: '深色',
      lightMode: '浅色',
      switch: '切换',
      currentLocale: '当前语言',
      sidebarCollapsed: '侧边栏收起',
      chatbarCollapsed: '聊天栏收起'
    },
    profile: {
      title: '个人资料',
      username: '用户名',
      nickname: '昵称',
      email: '邮箱',
      phone: '电话',
      qq: 'QQ 号码',
      avatar: '头像',
      emailInvalid: '请输入有效的邮箱地址',
      avatarHelp: '支持 PNG、JPG、GIF 或 WebP，最大 5MB。',
      avatarTooLarge: '头像文件过大（最大 5MB）',
      avatarUploadSuccess: '头像上传成功',
      avatarUploadFailed: '头像上传失败',
      address: '地址',
      introduction: '简介',
      save: '保存',
      reset: '重置',
      updated: '资料已更新',
      created: '资料已创建',
      saveFailed: '保存失败',
      loadFailed: '加载资料失败',
      roles: {
        superadmin: '超级管理员',
        admin: '管理员',
        user: '用户',
        unauthorized: '未授权'
      }
    },
    home: {
      greeting: '今天进展如何？',
      main: '主页'
    },
    alerts: {
      title: '告警',
      alertLabel: '告警 #{id}',
      add: '新增告警',
      search: '搜索告警',
      filterSeverity: '筛选严重程度',
      filterStatus: '筛选状态',
      filterAll: '全部',
      loading: '正在加载告警...',
      noAlerts: '暂无告警',
      noResults: '没有匹配的告警',
      retry: '重试',
      consult: 'AI 诊断',
      edit: '编辑',
      remove: '删除',
      createdAt: '创建时间',
      messageLabel: '消息',
      severityLabel: '严重级别',
      statusLabel: '状态',
      commentLabel: '备注/诊断',
      addTitle: '新增告警',
      editTitle: '编辑告警',
      enterMessage: '请输入告警信息',
      enterComment: '请输入详细说明或处理方案...',
      selectSeverity: '请选择严重级别',
      selectStatus: '请选择状态',
      cancel: '取消',
      update: '更新',
      create: '创建',
      confirmDelete: '确认删除',
      confirmDeleteText: '确定删除该告警吗？',
      aiTitle: 'AI 诊断',
      aiLoading: 'AI 分析中...',
      aiResponse: 'AI 响应',
      adoptTitle: '采纳 AI 建议',
      aiCommentPlaceholder: 'AI 分析摘要...',
      close: '关闭',
      severityDisaster: '灾难',
      severityCritical: '严重',
      severityHigh: '高',
      severityAverage: '平均',
      severityWarning: '警告',
      severityInfo: '信息',
      severityMedium: '中等',
      severityLow: '低',
      severityNotClassified: '未分类',
      statusOpen: '开启',
      statusAcknowledged: '已确认',
      statusResolved: '已解决',
      statusClosed: '已关闭',
      validationMessage: '请输入消息',
      validationSeverity: '请选择严重级别',
      validationStatus: '请选择状态'
    },
    hosts: {
      title: '终端资产',
      create: '新增资产',
      createTitle: '新增资产',
      search: '搜索资产',
      filterStatus: '筛选状态',
      filterMonitor: '筛选监控源',
      syncMonitor: '选择同步监控源',
      syncHosts: '同步主机',
      syncAll: '同步主机与指标',
      syncItems: '同步指标',
      filterAll: '全部',
      name: '名称',
      ip: 'IP 地址',
      hostId: '主机ID',
      description: '描述',
      status: '状态',
      lastSync: '最近同步',
      externalSource: '外部源',
      cancel: '取消',
      createBtn: '创建',
      loading: '正在加载主机...',
      noHosts: '暂无主机',
      noResults: '没有匹配的主机',
      retry: '重试',
      monitor: '监控源',
      actions: '操作',
      up: '在线',
      down: '离线',
      ai: 'AI',
      details: '详情',
      detailTitle: '主机详情',
      itemStatusChart: '指标状态概览',
      activeItemsTrend: '指标活跃趋势',
      compareTrends: '对比趋势',
      statusLegendTitle: '状态说明',
      importantOnly: '仅重要指标',
      itemList: '指标列表',
      activeItems: '活跃',
      items: '指标',
      properties: '属性',
      delete: '删除',
      propertiesTitle: '属性',
      save: '保存',
      saveAndPush: '保存并推送',
      confirm: '确认',
      deleteConfirmTitle: '警告',
      deleteConfirmText: '确定删除 {name} 吗？',
      validationName: '请输入主机名称',
      created: '主机创建成功',
      createFailed: '创建主机失败',
      updated: '主机更新成功',
      updateFailed: '更新主机失败',
      asyncPushStarted: '操作已启动。正在后台执行监控同步。',
      deleted: '主机删除成功',
      deleteFailed: '删除主机失败',
      deleteCanceled: '已取消删除',
      syncHostsSuccess: '主机同步完成',
      syncHostsFailed: '主机同步失败',
      syncAllSuccess: '主机和指标同步完成',
      syncAllFailed: '主机和指标同步失败',
      syncItemsSuccess: '{name} 指标同步完成',
      syncItemsFailed: '{name} 指标同步失败',
      pullHosts: '拉取主机',
      pullHostsSuccess: '主机拉取成功',
      pullHostsFailed: '拉取主机失败',
      pushHosts: '推送主机',
      pushHostsSuccess: '主机推送成功',
      pushHostsFailed: '推送主机失败',
      pullItems: '拉取指标',
      pullItemsSuccess: '已为 {name} 拉取指标',
      pullItemsFailed: '拉取 {name} 的指标失败',
      pushItems: '推送指标',
      pushItemsSuccess: '已为 {name} 推送指标',
      pushItemsFailed: '推送 {name} 的指标失败',
      terminal: 'WebSSH 终端',
      sshTitle: 'SSH 配置',
      sshUser: 'SSH 用户',
      sshPassword: 'SSH 密码',
      sshPort: 'SSH 端口',
      passwordPlaceholder: '留空表示不修改当前密码',
      aiError: '错误',
      aiFailed: 'AI 咨询失败',
      unknownError: '未知错误',
      aiTitle: 'AI 诊断',
      aiLoading: 'AI 分析中...',
      aiResponse: 'AI 响应',
      close: '关闭',
      unknown: '未知',
      cpuUsage: 'CPU 使用率',
      memoryUsage: '内存使用率',
      networkTraffic: '网络流量',
      reportGenerate: '生成报告',
      reportTitle: '主机报告',
      reportSummary: '概要',
      reportKeyMetrics: '关键指标',
      reportChatTitle: '对话精华',
      reportChatEmpty: '暂无对话精华',
      reportDisclaimer: '此报告基于当前主机指标和AI分析自动生成。关键决策请结合其他工具验证。',
      reportSuccess: '报告生成成功！',
      reportFailed: '生成报告失败',
      networkInfo: '网络信息',
      location: '归属地',
      geolocationLoading: '正在加载归属地...',
      geolocationFailed: '获取归属地失败'
    },
    monitors: {
      title: '资产来源',
      search: '搜索来源',
      searchField: '搜索字段',
      searchAll: '所有字段',
      filterStatus: '筛选状态',
      filterType: '筛选类型',
      filterAll: '全部',
      create: '新增监控',
      createTitle: '新增监控',
      name: '名称',
      url: 'URL',
      username: '用户名',
      password: '密码',
      type: '类型',
      description: '描述',
      noDescription: '暂无描述',
      status: '状态',
      cancel: '取消',
      createBtn: '创建',
      loading: '正在加载监控...',
      retry: '重试',
      noMonitors: '暂无监控',
      properties: '属性',
      delete: '删除',
      login: '登录',
      reLogin: '重新登录',
      syncGroups: '同步分组',
      propertiesTitle: '属性',
      save: '保存',
      unknown: '未知'
    },
    alarms: {
      title: '告警来源',
      create: '新增来源',
      createTitle: '新增来源',
      search: '搜索来源',
      searchField: '搜索字段',
      searchAll: '所有字段',
      filterStatus: '筛选状态',
      filterAll: '全部',
      name: '名称',
      url: 'URL',
      username: '用户名',
      password: '密码',
      type: '类型',
      description: '描述',
      noDescription: '暂无描述',
      status: '状态',
      cancel: '取消',
      createBtn: '创建',
      loading: '正在加载告警源...',
      retry: '重试',
      noAlarms: '暂无告警源',
      noResults: '没有匹配的告警源',
      properties: '属性',
      delete: '删除',
      login: '登录',
      reLogin: '重新登录',
      propertiesTitle: '属性',
      save: '保存',
      initializeZabbix: '初始化 Zabbix',
      initializeZabbixTooltip: '同步/更新 Zabbix 中的媒体类型和动作',
      unknown: '未知'
    },
    items: {
      back: '返回',
      total: '指标总数',
      details: '详情',
      detailTitle: '指标详情',
      statusChart: '状态图表',
      host: '主机ID',
      comment: '备注',
      titleAll: '指标库',
      titleHost: '资产 #{id} 的指标',
      titleHostName: '{name} 的指标',
      add: '新增指标',
      search: '搜索指标',
      filterHost: '筛选主机',
      filterStatus: '筛选状态',
      filterType: '筛选类型',
      filterAll: '全部',
      loading: '正在加载指标...',
      retry: '重试',
      noItems: '暂无监控指标',
      noResults: '没有匹配的指标',
      id: 'ID',
      name: '名称',
      value: '数值',
      units: '单位',
      type: '类型',
      interval: '间隔',
      status: '状态',
      enabled: '启用',
      disabled: '禁用',
      description: '描述',
      actions: '操作',
      ai: 'AI',
      edit: '编辑',
      delete: '删除',
      addTitle: '新增指标',
      editTitle: '编辑指标',
      enterName: '请输入指标名称',
      enterValue: '请输入指标值（如 cpu.usage）',
      selectType: '请选择类型',
      intervalSeconds: '间隔（秒）',
      statusEnabled: '启用',
      statusDisabled: '禁用',
      enterDescription: '请输入描述',
      cancel: '取消',
      update: '更新',
      create: '创建',
      validationName: '请输入名称',
      loadFailed: '加载指标失败',
      created: '指标创建成功',
      updated: '指标更新成功',
      saveFailed: '保存指标失败',
      deleted: '指标删除成功',
      deleteFailed: '删除指标失败',
      sync: '同步指标',
      syncSuccess: '指标同步完成',
      syncFailed: '指标同步失败',
      syncSelectHost: '请先选择主机',
      pull: '拉取指标',
      pullSuccess: '指标拉取成功',
      pullFailed: '拉取指标失败',
      pullSelectHost: '请先选择主机',
      push: '推送指标',
      saveAndPush: '保存并推送',
      createAndPush: '创建并推送',
      pushSuccess: '指标推送成功',
      pushFailed: '推送指标失败',
      pushSelectHost: '请先选择主机',
      selectedCount: '已选 {count} 项',
      bulkUpdate: '批量更新',
      bulkDelete: '批量删除',
      bulkUpdateTitle: '批量更新指标',
      bulkUpdateEnabled: '启用状态',
      bulkUpdateStatus: '状态',
      bulkUpdateNoChange: '不修改',
      bulkUpdateNoChanges: '请至少选择一个字段进行更新',
      bulkUpdateSuccess: '已更新 {count} 条指标',
      bulkUpdateFailed: '批量更新失败',
      bulkDeleteConfirmTitle: '确认批量删除',
      bulkDeleteConfirmText: '确定删除选中的 {count} 条指标吗？',
      bulkDeleteSuccess: '已删除 {count} 条指标',
      bulkDeleteFailed: '批量删除失败',
      selectAtLeastOne: '请至少选择一条指标',
      sort: '排序',
      sortNameAsc: '名称 A-Z',
      sortNameDesc: '名称 Z-A',
      sortCreatedDesc: '创建时间最新',
      sortUpdatedDesc: '最近更新',
      sortStatusDesc: '状态降序',
      sortStatusAsc: '状态升序',
      apply: '应用',
      aiError: '错误',
      aiFailed: 'AI 咨询失败',
      unknownError: '未知错误',
      confirmDelete: '确认删除',
      confirmDeleteText: '确定删除该指标吗？',
      aiTitle: 'AI 诊断',
      aiLoading: 'AI 分析中...',
      aiResponse: 'AI 响应',
      close: '关闭'
    },
    packets: {
      title: '报文分析',
      subtitle: '使用 AI 分析捕获的网络流量，检测异常或恶意会话流。',
      upload: '上传捕获',
      uploadTitle: '分析新报文/会话',
      name: '会话名称',
      provider: 'AI 供应商',
      model: '模型',
      file: 'PCAP/文本文件',
      rawContent: '原始十六进制/流数据 (可选)',
      rawContentPlaceholder: '请在此处粘贴十六进制或流数据...',
      analyze: '开始分析',
      status: '状态',
      risk: '风险等级',
      actions: '操作',
      reAnalyze: '重新分析',
      delete: '删除',
      noItems: '暂无报文分析记录。',
      analyzing: '分析中...',
      completed: '已完成',
      failed: '失败',
      pending: '等待中',
      clean: '安全的',
      notable: '值得关注',
      malicious: '恶意的',
      summary: '分析摘要',
      result: '结果',
      close: '关闭',
      success: '报文已上传并开始分析。',
      deleted: '记录已成功删除。',
      namePlaceholder: '例如：可疑的 TCP 会话流',
      providerLabel: '供应商',
      sourceFileLabel: '源文件',
      manualContent: '手动输入内容',
      rawDataTitle: '原始数据 / 会话流',
      aiAnalysisTitle: 'AI 智能分析',
      loadFailed: '加载数据失败',
      uploadFailed: '上传失败',
      analysisStarted: '分析已启动',
      startFailed: '启动分析失败',
      deleteFailed: '删除失败',
      unknown: '未知',
      nameRequired: '请输入名称',
      providerRequired: '请选择 AI 供应商',
      dropFile: '将文件拖到此处，或点击上传'
    },
    providers: {
      title: 'AI 与服务',
      create: '新增供应商',
      search: '搜索供应商',
      filterStatus: '筛选状态',
      filterType: '筛选类型',
      filterAll: '全部',
      sort: '排序',
      sortNameAsc: '名称 A-Z',
      sortNameDesc: '名称 Z-A',
      sortStatusAsc: '状态升序',
      sortStatusDesc: '状态降序',
      createTitle: '新增供应商',
      editTitle: '编辑供应商',
      loading: '正在加载供应商...',
      retry: '重试',
      noProviders: '暂无供应商',
      noResults: '没有匹配的供应商',
      statusOperational: '运行中',
      statusOffline: '离线',
      properties: '属性',
      delete: '删除',
      type: '类型',
      providerName: '供应商名称',
      url: 'URL',
      apiKey: 'API Key',
      defaultModel: '默认模型',
      description: '描述',
      status: '状态',
      cancel: '取消',
      edit: '编辑',
      createBtn: '创建',
      propertiesTitle: '属性',
      name: '名称',
      model: '模型',
      fetchModels: '获取可用模型',
      save: '保存',
      nameRequired: '请输入供应商名称',
      apiKeyRequired: '请输入 API Key',
      typeRequired: '请选择供应商类型',
      typeGemini: 'Gemini',
      typeOpenAI: 'OpenAI',
      typeOther: '其他 (OpenAI 兼容)',
      checkStatus: '检查状态',
      checkAll: '检查全部',
    },
    users: {
      title: '身份管理',
      search: '搜索用户',
      create: '新增用户',
      loading: '正在加载用户...',
      retry: '重试',
      noUsers: '暂无用户',
      id: 'ID',
      username: '用户名',
      nickname: '昵称',
      email: '邮箱',
      phone: '电话',
      qq: 'QQ',
      role: '角色',
      status: '状态',
      active: '启用',
      inactive: '停用',
      actions: '操作',
      edit: '编辑',
      delete: '删除',
      createTitle: '新增用户',
      editTitle: '编辑用户',
      password: '密码',
      passwordHint: '留空表示不修改',
      avatar: '头像',
      address: '地址',
      introduction: '简介',
      cancel: '取消',
      update: '更新',
      createBtn: '创建',
      confirmDelete: '确认删除',
      confirmDeleteText: '确定删除该用户吗？',
      usernameRequired: '请输入用户名',
      passwordRequired: '请输入密码',
      created: '用户已创建',
      updated: '用户已更新',
      deleted: '用户已删除',
      loadFailed: '加载用户失败',
      loadInfoFailed: '加载用户信息失败',
      operationFailed: '操作失败',
      deleteFailed: '删除失败',
      roleUnauthorized: '未授权',
      roleUser: '普通用户',
      roleAdmin: '管理员',
      roleSuper: '超级管理员'
    },
    menu: {
      databoard: '大盘概况',
      insights: '智能洞察',
      systemStatus: '系统健康',
      metricRacing: '指标赛跑',
      analytics: '趋势分析',
      reports: '运维报告',
      inventory: '资产盘点',
      group: '主机分组',
      host: '终端资产',
      observability: '数据监控',
      item: '指标库',
      alerting: '告警响应',
      alert: '实时告警',
      automation: '自动规则',
      trigger: '触发策略',
      action: '执行动作',
      sources: '数据接入',
      monitor: '资产来源',
      alarm: '告警来源',
      tooling: '处置工具',
      terminal: '远程控制',
      kb: '知识库',
      communication: '通知渠道',
      media: '通知对象',
      provider: 'AI 与服务',
      providers: '服务商管理',
      system: '平台设置',
      user: '身份管理',
      registerApplication: '用户申请',
      log: '系统日志',
      auditLog: '审计日志',
      configuration: '系统配置',
      profile: '个人资料',
      about: '关于',
      maintenance: '运维服务',
      delivery: '告警投递',
      users: '用户管理',
    },
    media: {
      title: '媒介',
      create: '创建媒介',
      createTitle: '创建媒介',
      search: '搜索媒介',
      filterStatus: '筛选状态',
      filterAll: '全部',
      name: '名称',
      type: '类型',
      typeHint: 'email/other',
      target: '目标',
      targetHint: '地址或端点',
      params: '参数',
      paramsHint: '用于模板的 JSON 参数',
      description: '描述',
      status: '状态',
      actions: '操作',
      test: '测试',
      testSuccess: '测试消息发送成功',
      testFailed: '测试失败',
      properties: '属性',
      save: '保存',
      cancel: '取消',
      delete: '删除',
      loading: '正在加载媒介...',
      loadFailed: '加载媒介失败',
      noMedia: '暂无媒介',
      noResults: '没有匹配的媒介',
      retry: '重试',
      created: '媒介创建成功！',
      updated: '媒介更新成功！',
      deleted: '媒介删除成功！',
      createFailed: '创建媒介失败',
      updateFailed: '更新媒介失败',
      deleteFailed: '删除媒介失败',
      deleteCanceled: '已取消删除',
      validationName: '请输入媒介名称',
      validationType: '请选择媒介类型',
      unknownType: '未知类型'
    },
    actions: {
      title: '动作',
      create: '创建动作',
      createTitle: '创建动作',
      search: '搜索动作',
      filterStatus: '筛选状态',
      filterAll: '全部',
      name: '名称',
      severity: '严重级别',
      severityMin: '最低严重级别',
      alertStatus: '告警状态',
      host: '主机',
      group: '分组',
      trigger: '触发策略',
      media: '媒介',
      template: '模板',
      templateHint: '可用 {{alert_id}} {{message}} {{severity}} {{status}} {{host_id}} {{host_name}} {{item_id}} {{name}} {{value}} {{units}} {{monitor_id}} {{group_id}} {{entity}} {{added}} {{updated}} {{failed}} {{total}}',
      description: '描述',
      status: '状态',
      actions: '操作',
      properties: '属性',
      save: '保存',
      cancel: '取消',
      delete: '删除',
      loading: '正在加载动作...',
      loadFailed: '加载动作失败',
      noActions: '暂无动作',
      noResults: '没有匹配的动作',
      retry: '重试',
      created: '动作创建成功！',
      updated: '动作更新成功！',
      deleted: '动作删除成功！',
      createFailed: '创建动作失败',
      updateFailed: '更新动作失败',
      deleteFailed: '删除动作失败',
      deleteCanceled: '已取消删除',
      validationName: '请输入动作名称',
      unknownMedia: '未知媒介',
      test: '测试',
      testSuccess: '已成功请求测试动作',
      testFailed: '测试动作失败'
    },
    triggers: {
      title: '触发策略',
      create: '创建策略',
      createTitle: '创建策略',
      search: '搜索策略',
      filterStatus: '筛选状态',
      filterAll: '全部',
      name: '名称',
      entity: '实体',
      entityAlert: '告警',
      entityLog: '日志',
      entityItem: '指标',
      severity: '严重级别',
      severityMin: '最低严重级别',
      alertStatus: '告警状态',
      alertQuery: '告警查询',
      alertId: '告警ID',
      alertMonitorId: '监控ID',
      alertGroupId: '分组ID',
      alertHostId: '主机ID',
      alertItemId: '指标ID',
      hostName: '主机',
      itemName: '指标项',
      itemStatus: '指标状态',
      itemHostId: '主机名',
      itemId: '指标名',
      itemValueOperator: '数值运算符',
      itemValueOperatorHint: '选择运算符',
      itemValueThreshold: '数值阈值',
      monitorField: '监控字段',
      monitorFieldStatus: '指标状态',
      monitorFieldValue: '指标数值',
      operatorBetween: '区间内',
      operatorOutside: '区间外',
      action: '动作',
      status: '状态',
      actions: '操作',
      properties: '属性',
      save: '保存',
      cancel: '取消',
      delete: '删除',
      loading: '正在加载策略...',
      loadFailed: '加载策略失败',
      noTriggers: '暂无策略',
      noResults: '没有匹配的策略',
      retry: '重试',
      created: '策略创建成功！',
      updated: '策略更新成功！',
      deleted: '策略删除成功！',
      createFailed: '创建策略失败',
      updateFailed: '更新策略失败',
      deleteFailed: '删除策略失败',
      deleteCanceled: '已取消删除',
      validationName: '请输入策略名称',
      validationAction: '请选择动作',
      unknownAction: '未知动作'
    },
    logs: {
      title: '日志',
      systemTitle: '系统日志',
      serviceTitle: '服务日志',
      search: '搜索日志',
      filterLevel: '筛选严重级别',
      filterAll: '全部',
      level: '严重级别',
      levelInfo: '信息',
      levelWarn: '警告',
      levelAverage: '平均',
      levelError: '错误',
      levelCritical: '严重',
      levelFatal: '致命',
      message: '消息',
      context: '上下文',
      ip: 'IP',
      createdAt: '创建时间',
      refresh: '刷新',
      loading: '正在加载日志...',
      retry: '重试',
      noLogs: '暂无日志',
      loadFailed: '加载日志失败',
      details: '日志详情',
      clear: '清空日志',
      export: '导出 CSV',
      autoRefresh: '自动刷新',
      rawContext: '原始上下文',
      formattedContext: '格式化上下文',
      retry: '重试',
      applyAndSave: '应用并保存'
    },
    groups: {
      title: '主机分组',
      create: '创建分组',
      createTitle: '创建分组',
      search: '搜索分组',
      filterStatus: '筛选状态',
      filterAll: '全部',
      name: '名称',
      description: '描述',
      status: '状态',
      enabled: '启用',
      hosts: '主机',
      items: '指标',
      actions: '操作',
      details: '详情',
      properties: '属性',
      save: '保存',
      cancel: '取消',
      delete: '删除',
      loading: '正在加载分组...',
      loadFailed: '加载分组失败',
      noGroups: '暂无分组',
      noResults: '没有匹配的分组',
      retry: '重试',
      created: '分组创建成功！',
      updated: '分组更新成功！',
      deleted: '分组删除成功！',
      createFailed: '创建分组失败',
      updateFailed: '更新分组失败',
      deleteFailed: '删除分组失败',
      deleteCanceled: '已取消删除',
      pull: '拉取分组',
      pullSuccess: '分组拉取成功',
      pullFailed: '拉取分组失败',
      push: '推送分组',
      pushSuccess: '分组推送成功',
      pushFailed: '推送分组失败',
      detailTitle: '分组详情',
      summary: '概览',
      totalHosts: '主机总数',
      activeHosts: '活跃主机',
      errorHosts: '错误主机',
      syncingHosts: '同步中主机',
      totalItems: '指标总数',
      selectGroup: '请选择一个分组查看详情',
      hostStatusChart: '主机状态概览'
    },
    configuration: {
      title: '系统配置',
      subtitle: '管理系统全局设置与集成',
      system: '系统设置',
      database: '数据库设置',
      services: '后台服务',
      syncService: '同步服务',
      statusCheckService: '状态检查服务',
      integrations: '集成',
      aiAnalysis: 'AI 分析',
      onebotSettings: 'OneBot (NapCat) 配置',

      mcpSettings: 'MCP 设置',
      advanced: '高级设置',
      siteMessageSettings: '站内消息设置',
      minAlertSeverity: '最低告警严重级别',
      minLogSeverity: '最低日志严重级别',
      mediaRateLimitSettings: '通知频率限制设置',
      globalInterval: '全局间隔 (秒)',
      protocolInterval: '协议间隔 (秒)',
      mediaInterval: '媒体间隔 (秒)',
      onebotEnabled: '启用 OneBot',
      onebotMode: '连接模式',
      onebotModeReverse: '反向 (NapCat 作为客户端)',
      onebotModePositive: '正向 (服务器作为客户端)',
      onebotUrl: 'WebSocket 地址',
      onebotToken: '访问令牌',

      external: '外部设施',
      mcpServers: 'MCP 服务列表',
      addMcpServer: '添加 MCP 服务',
      mcpServerName: '服务名称',
      mcpServerCommand: '启动命令',
      mcpServerApiKey: 'API Key',
      mcpServerApiKeyPlaceholder: '需要鉴权的服务可填写',
      mcpServerApiKeyEnv: 'API Key 环境变量',
      mcpServerArgs: '命令参数（每行一个）',
      mcpServerArgsPlaceholder: '-y\n@modelcontextprotocol/server-filesystem\nD:/Nagare_Project',
      mcpServerEnabled: '启用',
      mcpServerTest: '测试',
      mcpServerHelpText: '每个服务会以本地 MCP stdio 进程启动。保存配置后将按新设置自动重连。',
      testConnection: '连接测试',
      mcpTestSuccess: '连接测试成功，已加载工具数',
      mcpTestFailed: '连接测试失败',
      mcpClientStatus: 'MCP 客户端状态',
      refreshMcpStatus: '刷新状态',
      mcpStatusTotal: '已配置',
      mcpStatusConnected: '已连接',
      mcpServerState: '状态',
      mcpStateConnected: '已连接',
      mcpStateDisconnected: '未连接',
      mcpServerToolCount: '工具数',
      mcpServerUpdatedAt: '更新时间',
      mcpServerLastError: '最近错误',
      mcpStatusLoadFailed: '加载 MCP 客户端状态失败',
      unknownError: '未知错误',
      externalInfrastructure: '外部基础设施定义',
      addItem: '添加定义',
      itemType: '类型',
      itemKey: '标识(Key)',
      itemName: '名称',
      itemId: '数字ID',
      itemActions: '操作',
      itemHelpText: '这些定义决定了监控、告警、AI 供应商和通知媒介的可选类型。',
      systemName: '系统名称',
      ipAddress: 'IP 地址',
      port: '端口',
      availability: '可用性',
      dbVersion: '数据库版本',
      dbHost: '数据库主机',
      dbPort: '数据库端口',
      dbUsername: '数据库用户名',
      dbPassword: '数据库密码',
      dbName: '数据库名称',
      save: '保存配置',
      reload: '重新加载配置',
      reset: '重置为默认',
      cancel: '取消',
      edit: '编辑',
      saving: '正在保存...',
      loading: '正在加载配置...',
      saveSuccess: '配置保存成功！',
      saveFailed: '配置保存失败',
      reloadSuccess: '配置重新加载成功！',
      reloadFailed: '配置重新加载失败',
      confirmSave: '保存更改？',
      confirmSaveText: '您确定要保存这些配置更改吗？',
      resetConfirmTitle: '重置为默认值？',
      resetConfirmText: '这将把所有配置项重置为系统默认值。此操作无法撤销。您确定吗？',
      resetConfirmYes: '是的，全部重置',
      resetConfirmNo: '不，保留我的设置',
      resetSuccess: '配置已重置为默认值。',
      yes: '是',
      no: '否'
    },
    kb: {
      title: '知识库',
      subtitle: '管理用于 RAG 告警分析的本地知识',
      searchPlaceholder: '搜索主题、内容或关键词...',
      create: '添加知识',
      createTitle: '新增知识库条目',
      editTitle: '编辑知识库条目',
      topic: '主题',
      category: '分类',
      keywords: '关键词',
      content: '内容',
      noItems: '未找到知识库条目',
      loadFailed: '加载知识库失败',
      deleteConfirm: '确定要删除这条知识吗？',
      deleteSuccess: '条目已删除',
      deleteFailed: '删除条目失败',
      createSuccess: '条目已创建',
      updateSuccess: '条目已更新',
      saveFailed: '保存条目失败',
      topicRequired: '请输入主题',
      contentRequired: '请输入内容',
      categoryRequired: '请选择分类'
    },
    reports: {
      title: '报表',
      subtitle: '自动化运维报告',
      filterType: '筛选类型',
      typeDaily: '日报',
      typeWeekly: '周报',
      typeMonthly: '月报',
      typeCustom: '自定义',
      config: '配置',
      configTitle: '报表配置',
      generateDaily: '生成日报',
      generateWeekly: '生成周报',
      generateMonthly: '生成月报',
      generateCustom: '生成自定义报表',
      customTitle: '生成自定义报表',
      titleLabel: '标题',
      rangeLabel: '时间范围',
      autoGenerate: '自动生成',
      generateDay: '生成日',
      generateTime: '生成时间',
      generateDate: '生成日期',
      reportTitle: '标题',
      type: '类型',
      generatedAt: '生成时间',
      status: '状态',
      loadFailed: '加载报表失败',
      generationStarted: '报表生成已开始',
      generationFailed: '报表生成失败',
      deleteConfirm: '确定要删除这份报表吗？',
      deleteSuccess: '报表删除成功',
      language: '报告语言',
      customDefaultTitle: '自定义基础设施报告',
      enableAISummary: 'AI 摘要生成',
    },
    message: {
      title: '系统通知',
      empty: '暂无新通知',
      markAllRead: '全部标记为已读',
      markAllReadSuccess: '已将所有通知标记为已读',
      viewAll: '查看全部历史',
      historyTitle: '通知历史',
      historySubtitle: '查看并管理您的系统通知',
      filterAll: '全部',
      filterUnread: '未读',
      colTitle: '标题',
      colContent: '内容',
      colType: '类型',
      colTime: '时间',
      loadFailed: '加载通知失败',
      deleteConfirm: '确定要删除这条通知吗？',
      deleteSuccess: '通知已删除',
      deleteFailed: '删除通知失败',
      operationFailed: '操作失败'
    },
    terminal: {
      title: '终端',
      selectHost: '请选择主机',
      switchHost: '切换主机',
      disconnect: '断开连接',
      reconnect: '重新连接',
      connectTitle: '连接到主机',
      savedHost: '已保存主机',
      directConnect: '直接连接',
      selectHostPlaceholder: '请选择一个主机',
      connect: '连接',
      noCredentials: '该主机未配置 SSH 凭据。请先更新主机属性。',
      fetchInfoFailed: '获取主机信息失败',
      connected: '已连接到主机',
      closed: '连接已关闭',
      wsError: 'WebSocket 错误',
      connFailed: 'WebSocket 连接失败。请确保主机可达且 SSH 凭据正确。'
    },
    auditLog: {
      title: '审计日志',
      search: '搜索用户操作...'
    },
    analytics: {
      title: '趋势分析',
      subtitle: '来自监控数据的见解和统计趋势',
      chaosTitle: '混沌模拟器',
      chaosDesc: '触发模拟告警风暴，测试系统抗压能力。',
      triggerChaos: '触发告警风暴',
      alertTrend: '告警趋势 (最近 14 天)',
      severityDist: '严重级别分布',
      topKeywords: '告警关键词排行',
      alertIntensity: '告警强度 (热力图)',
      topNoisyHosts: '告警主机排行',
      alerts: '告警数',
      totalAlerts: '累计告警',
      systemHealth: '系统健康度',
      activeHosts: '活动资产'
    },
    metricRacing: {
      title: '指标赛跑',
      subtitle: '基础设施资产的实时性能排行',
      cpuTitle: 'CPU 利用率 (%)',
      netTitle: '网络流量',
      itemTitle: '指标数值',
      start: '开始赛跑',
      stop: '停止赛跑'
    },
    retention: {
      title: '数据保留',
      subtitle: '管理系统数据的保留时间',
      dataType: '数据类型',
      retentionDays: '保留天数',
      enabled: '已启用',
      description: '描述',
      actions: '操作',
      edit: '编辑',
      save: '保存',
      cancel: '取消',
      manualCleanup: '手动清理',
      nextCleanup: '下次清理时间：今日 02:00 AM',
      performCleanup: '立即清理',
      cleanupConfirm: '手动清理',
      cleanupConfirmText: '您确定要立即根据启用的策略执行手动数据清理吗？',
      cleanupSuccess: '清理成功',
      cleanupSuccessMsg: '清理执行成功。',
      recordsRemoved: '已移除记录：',
      noRecordsCleaned: '没有需要清理的记录。',
      cleanupFailed: '清理失败',
      cleanupFailedMsg: '清理操作失败。',
      autoRefreshing: '自动刷新中 (30s)',
      enterDescription: '请输入描述',
      updateSuccess: '策略更新成功',
      updateFailed: '策略更新失败',
      forever: '永久保留',
      automatedCleanupTitle: '自动清理',
      automatedCleanupDesc: '系统每天凌晨 2:00 会根据这些策略自动执行数据保留清理任务。',
      types: {
        logs: '系统日志',
        alerts: '告警记录',
        audit_logs: '审计日志',
        item_history: '监控项历史',
        host_history: '主机历史',
        network_history: '网络健康历史',
        chat: 'AI 聊天历史',
        reports: '生成的报表',
        site_messages: '站内消息',
        ansible_jobs: 'Ansible 任务',
        group_history: '分组历史',
        monitor_history: '监控系统历史'
      },
      typeDescriptions: {
        logs: '系统与服务的运行日志。',
        alerts: '系统告警与事件的历史记录。',
        audit_logs: '用户操作与安全性审计日志。',
        item_history: '监控项指标的时间序列历史数据。',
        host_history: '主机的状态变化与可用性历史。',
        network_history: '全局网络健康评分的历史趋势。',
        chat: '用户与 AI 助手的对话聊天记录。',
        reports: '系统自动或手动生成的 PDF 运维报表。',
        site_messages: '发送给用户的内部通知与站内消息。',
        ansible_jobs: '已执行的 Ansible Playbook 与任务的历史记录。',
        group_history: '主机分组的状态变化与可用性历史。',
        monitor_history: '监控系统的状态变化与可用性历史。'
      }
    },
    about: {
      title: '关于 Nagare',
      subtitle: '开源基础设施智能平台',
      description: 'Nagare 是一个深度基础设施洞见与自动化运维平台，旨在为现代环境提供全面的可见性和 AI 辅助的故障排查。',
      disclaimer: '由于 Nagare 仍处于积极开发阶段，部分功能和系统稳定性可能尚不完善。我们非常感谢您的使用与分享，并对开发初期可能给您带来的不便深表歉意。',
      contact: '如果您遇到任何问题，欢迎通过以下方式联系：',
      github: 'GitHub 仓库',
      author: '作者',
      email: '电子邮箱',
      version: '版本',
      license: '许可协议'
    }
  }
}

// Add aliases for broader compatibility
messages['zh-CN'] = messages.zh

const i18n = createI18n({
  legacy: false,
  globalInjection: true,
  locale: savedLocale,
  fallbackLocale: 'en',
  messages
})

export default i18n
