# Nagare Project Overview

## Purpose
Nagare is a monitoring management platform with a Go backend (API server) and a Vue 3 frontend (web UI). It centralizes monitors, hosts, items, alerts, media providers, and actions, and exposes both a human UI and programmatic APIs for operations and integrations.

## Architecture At A Glance
- Backend: Go (Gin) HTTP server under nagare-v0.21.
- Frontend: Vue 3 + Vite SPA under nagare_web.
- API base path: /api/v1.
- Auth model: JWT-based with privilege levels enforced by middleware.
- Integrations: QQ (OneBot 11) message webhook, MCP server endpoints, and media providers.
...
- Privilege 3: super admin (configuration and user administration).
- Public endpoints: selected webhook and system endpoints are intentionally unauthenticated.
- MCP endpoints: protected by an API key middleware.
...
All routes are in the SPA router and map to Vue view components.
...
- Supports commands: /status, /get_alert, /chat <message>
...
### Backend
- Main server entry: nagare-v0.21/cmd/web_server/main.go
- Router definition: nagare-v0.21/cmd/web_server/router/router.go
- Config file: nagare-v0.21/configs/nagare_config.json

## Source Map
- Backend routes: nagare-v0.21/cmd/web_server/router/router.go
- Frontend routes: nagare_web/src/router/index.js
