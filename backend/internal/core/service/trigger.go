...
func SearchTriggersServ(filter domain.TriggerFilter) ([]TriggerResp, error) {
...
func CountTriggersServ(filter domain.TriggerFilter) (int64, error) {
...
func AddTriggerServ(req TriggerReq) (TriggerResp, error) {
	...
	if action, err := repository.GetActionByIDDAO(req.ActionID); err == nil {
		...
		trigger.Status = determineTriggerStatus(trigger, domain.Action{})
	}
...
func UpdateTriggerServ(id uint, req TriggerReq) error {
	...
	if req.Enabled != existing.Enabled || req.ActionID != existing.ActionID {
		if action, err := repository.GetActionByIDDAO(req.ActionID); err == nil {
			...
			updated.Status = determineTriggerStatus(updated, domain.Action{})
		}
	}
...
func triggerToResp(trigger domain.Trigger) TriggerResp {
	return TriggerResp{
		ID:                 int(trigger.ID),
		Name:               trigger.Name,
...
func ExecuteTriggersForAlert(alert domain.Alert) {
...
func ExecuteTriggersForLog(entry domain.LogEntry) {
...
func ExecuteTriggersForItem(item domain.Item) {
...
func buildLogReplacements(entry domain.LogEntry) map[string]string {
	return map[string]string{
...
func buildItemReplacements(item domain.Item) map[string]string {
	hostName := ""
...
func execTriggersForLog(entry domain.LogEntry, replacements map[string]string) {
...
func execTriggersForItem(item domain.Item, replacements map[string]string) {
...
func invokeAlertTriggerAction(trigger domain.Trigger, replacements map[string]string) {
...
func invokeLogTriggerAction(trigger domain.Trigger, replacements map[string]string) {
	if trigger.Enabled == 0 {
		return
	}
	action, err := repository.GetActionByIDDAO(trigger.ActionID)
...
func invokeItemTriggerAction(trigger domain.Trigger, replacements map[string]string) {
	if trigger.Enabled == 0 {
		return
	}
	action, err := repository.GetActionByIDDAO(trigger.ActionID)
...
func buildAlertMatchContext(alert domain.Alert) alertMatchContext {
...
func matchAlertTrigger(trigger domain.Trigger, ctx alertMatchContext) bool {
...
func matchLogTrigger(trigger domain.Trigger, entry domain.LogEntry) bool {
	entity := normalizeTriggerEntity(trigger.Entity)
	if entity != "log" {
		return false
	}
...
func matchItemTrigger(trigger domain.Trigger, item domain.Item) bool {
	entity := normalizeTriggerEntity(trigger.Entity)
	if entity != "item" {
		return false
	}
