...
func recordItemHistory(item domain.Item, sampledAt time.Time) {
	if sampledAt.IsZero() {
		sampledAt = time.Now().UTC()
	}
	_ = repository.AddItemHistoryDAO(domain.ItemHistory{
		ItemID:    item.ID,
		HostID:    item.HID,
...
func recordHostHistory(host domain.Host, sampledAt time.Time) {
	if sampledAt.IsZero() {
		sampledAt = time.Now().UTC()
	}
	_ = repository.AddHostHistoryDAO(domain.HostHistory{
		HostID:            host.ID,
		Status:            host.Status,
...
func recordNetworkStatusSnapshot(sampledAt time.Time) {
	...
	_ = repository.AddNetworkStatusHistoryDAO(domain.NetworkStatusHistory{
		Score:         score.Score,
...
func reverseItemHistory(rows []domain.ItemHistory) {
	for i, j := 0, len(rows)-1; i < j; i, j = i+1, j-1 {
		rows[i], rows[j] = rows[j], rows[i]
	}
}

func reverseHostHistory(rows []domain.HostHistory) {
	for i, j := 0, len(rows)-1; i < j; i, j = i+1, j-1 {
		rows[i], rows[j] = rows[j], rows[i]
	}
}

func reverseNetworkStatusHistory(rows []domain.NetworkStatusHistory) {
	for i, j := 0, len(rows)-1; i < j; i, j = i+1, j-1 {
		rows[i], rows[j] = rows[j], rows[i]
	}
}
