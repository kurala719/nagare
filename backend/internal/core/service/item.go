...
func SearchItemsServ(filter domain.ItemFilter) ([]ItemResp, error) {
...
func CountItemsServ(filter domain.ItemFilter) (int64, error) {
...
func AddItemServ(req ItemReq) (ItemResp, error) {
	...
	if loadedHost, err := repository.GetHostByIDDAO(hid); err == nil {
		...
		item.Status = determineItemStatus(item, domain.Host{Enabled: 0, Status: 2})
	}
...
func UpdateItemServ(id uint, req ItemReq) error {
	...
	if req.Enabled != existing.Enabled {
		if host, err := repository.GetHostByIDDAO(hid); err == nil {
			...
			updated.Status = determineItemStatus(updated, domain.Host{Enabled: 0, Status: 2})
		}
...
func AddItemsByHostIDFromMonitorServ(hid uint) error {
	...
	for _, item := range monitorItems {
		enabled, status := mapMonitorItemStatus(item.Status)
		if err := repository.AddItemDAO(domain.Item{
			Name:           item.Name,
			HID:            hid,
			ItemID:         item.ID,
...
// createMonitorClientFromDomain creates a monitor client from a domain.Monitor
func createMonitorClientFromDomain(monitor domain.Monitor) (*monitors.Client, error) {
	cfg := monitors.Config{
		Name: monitor.Name,
...
func itemToResp(item domain.Item) ItemResp {
	return ItemResp{
		ID:             item.ID,
		Name:           item.Name,
		HID:            item.HID,
...
func pullItemsFromHostServ(mid, hid uint, recordHistory bool) (SyncResult, error) {
	...
	if monitors.ParseMonitorType(monitor.Type) == monitors.MonitorSNMP {
		...
		if err == nil {
			nameToItems := make(map[string][]domain.Item)
