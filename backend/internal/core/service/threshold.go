...
func evaluateThreshold(host domain.Host, item domain.Item) {
...
func triggerAlert(host domain.Host, item domain.Item, message string, severity int) {
	// Simple suppression: don't create same alert for same item if one exists in last 1 hour
	recentAlerts, err := repository.SearchAlertsDAO(domain.AlertFilter{
