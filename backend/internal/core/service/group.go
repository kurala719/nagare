...
func SearchGroupsServ(filter domain.GroupFilter) ([]GroupResp, error) {
	groupFilter := domain.GroupFilter{
		Query:     filter.Query,
...
func CountGroupsServ(filter domain.GroupFilter) (int64, error) {
	groupFilter := domain.GroupFilter{
		Query:     filter.Query,
		Status:    filter.Status,
		MonitorID: filter.MonitorID,
...
func AddGroupServ(req GroupReq) (GroupResp, error) {
	...
	groups, err := repository.SearchGroupsDAO(domain.GroupFilter{Query: group.Name})
...
func UpdateGroupServ(id uint, req GroupReq) error {
	...
	hosts, err := repository.SearchHostsDAO(domain.HostFilter{GroupID: &id})
	if err != nil {
		return err
	}
...
func groupToResp(group domain.Group) GroupResp {
	return GroupResp{
		ID:             int(group.ID),
		Name:           group.Name,
