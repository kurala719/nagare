...
func SearchActionsServ(filter domain.ActionFilter) ([]ActionResp, error) {
...
func CountActionsServ(filter domain.ActionFilter) (int64, error) {
...
func AddActionServ(req ActionReq) (ActionResp, error) {
	...
	if media, err := repository.GetMediaByIDDAO(req.MediaID); err == nil {
		...
		action.Status = determineActionStatus(action, domain.Media{})
	}
...
func UpdateActionServ(id uint, req ActionReq) error {
	...
	if req.Enabled != existing.Enabled || req.MediaID != existing.MediaID {
		if media, err := repository.GetMediaByIDDAO(req.MediaID); err == nil {
			...
			updated.Status = determineActionStatus(updated, domain.Media{})
		}
	}
...
func actionToResp(action domain.Action) ActionResp {
	return ActionResp{
		ID:          int(action.ID),
		Name:        action.Name,
...
func ExecuteAction(action domain.Action, media domain.Media, replacements map[string]string) error {
	msg := action.Template
	if msg == "" {
		msg = "Alert: {{message}}"
	}
...
func ExecuteLogAction(action domain.Action, media domain.Media, replacements map[string]string) error {
	msg := action.Template
	if msg == "" {
		msg = "Log: {{message}}"
	}
...
func sendMediaMessage(media domain.Media, msg string) error {
...
func resolveMediaTypeKeyForSend(media domain.Media) string {
