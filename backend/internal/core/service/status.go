...
func determineMonitorStatus(m domain.Monitor) int {
	if m.Enabled == 0 {
		return 0
	}
...
func determineAlarmStatus(a domain.Alarm) int {
	if a.Enabled == 0 {
		return 0
	}
	if a.AuthToken != "" {
...
func determineProviderStatus(p domain.Provider) int {
	if p.Enabled == 0 {
		return 0
	}
	if p.APIKey == "" {
...
func determineHostStatus(h domain.Host, monitor domain.Monitor) int {
	if h.Enabled == 0 {
		return 0
	}
...
func determineItemStatus(i domain.Item, host domain.Host) int {
	if i.Enabled == 0 {
		return 0
	}
...
func determineMediaStatus(m domain.Media) int {
	if m.Enabled == 0 {
		return 0
	}
	if m.Type == "" || m.Target == "" {
...
func determineActionStatus(a domain.Action, media domain.Media) int {
	if a.Enabled == 0 {
		return 0
	}
...
func determineTriggerStatus(t domain.Trigger, action domain.Action) int {
	if t.Enabled == 0 {
		return 0
	}
...
func determineGroupStatus(group domain.Group, hosts []domain.Host) int {
	if group.Enabled == 0 {
		return 0
	}
...
func recomputeGroupStatus(gid uint) (int, error) {
	...
	hosts, err := repository.SearchHostsDAO(domain.HostFilter{GroupID: &gid})
	if err != nil {
		return 0, err
	}
...
func recomputeHostStatus(hid uint) (int, error) {
	...
	status := 0
	if err != nil {
		status = determineHostStatus(host, domain.Monitor{Enabled: 0, Status: 2})
		if status == 2 {
...
func recomputeItemStatus(id uint) (int, error) {
	...
	if err != nil {
		status := determineItemStatus(item, domain.Host{Enabled: 0, Status: 2})
		if status == 2 {
...
func recomputeMonitorRelated(mid uint) error {
	...
	// 2. Recompute Groups
	groups, err := repository.SearchGroupsDAO(domain.GroupFilter{MonitorID: &mid})
	if err == nil {
...
func recomputeActionStatus(id uint) (int, error) {
	...
	if err != nil {
		status := determineActionStatus(action, domain.Media{})
...
func recomputeTriggerStatus(id uint) (int, error) {
	...
	if err != nil {
		status := determineTriggerStatus(trigger, domain.Action{})
...
func RecomputeAllStatuses() error {
	...
	hosts, err := repository.SearchHostsDAO(domain.HostFilter{})
	if err == nil {
		for _, h := range hosts {
