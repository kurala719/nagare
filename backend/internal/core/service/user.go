...
func SearchUsersServ(filter domain.UserFilter) ([]UserResponse, error) {
...
func CountUsersServ(filter domain.UserFilter) (int64, error) {
...
func RegisterUserServ(req RegisterRequest) error {
	...
	if err := repository.CreateRegisterApplicationDAO(domain.RegisterApplication{
		Username: req.Username,
		Password: req.Password,
		Email:    req.Email,
		Status:   0,
...
func userToResp(u domain.User) UserResponse {
	return UserResponse{
		ID:           int(u.ID),
		Username:     u.Username,
