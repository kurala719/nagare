...
func GetAllChatsServ() ([]domain.Chat, error) {
	return repository.GetAllChatsDAO()
}

// GetChatsWithPaginationServ retrieves chat history with pagination
func GetChatsWithPaginationServ(limit, offset int) ([]domain.Chat, error) {
	if limit <= 0 || limit > 50 {
		limit = 10
	}
	if offset < 0 {
...
func SearchChatsServ(filter domain.ChatFilter) ([]domain.Chat, error) {
...
func analyzeNetworkStatus(req ChatReq) (ChatRes, error) {
	...
	status := 0
	limit := 50
	alertFilter := domain.AlertFilter{Status: &status, Limit: limit}
...
func sendChatPlain(req ChatReq, personaPrompt string) (ChatRes, error) {
	...
	if err := repository.AddChatDAO(domain.Chat{
		UserID:     1,
