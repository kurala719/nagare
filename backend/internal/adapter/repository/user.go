...
func GetAllUsersDAO() ([]domain.User, error) {
...
func SearchUsersDAO(filter domain.UserFilter) ([]domain.User, error) {
...
func CountUsersDAO(filter domain.UserFilter) (int64, error) {
	query := database.DB.Model(&domain.User{})
	if filter.Query != "" {
		q := "%" + filter.Query + "%"
		query = query.Where("username LIKE ? OR email LIKE ? OR nickname LIKE ? OR phone LIKE ?", q, q, q, q)
...
func GetUserByIDDAO(id int) (domain.User, error) {
...
func GetUserByUsernameDAO(username string) (domain.User, error) {
	var user domain.User
...
func AddUserDAO(user domain.User) error {
...
func UpdateUserDAO(id int, user domain.User) error {
	...
	return database.DB.Model(&domain.User{}).Where("id = ?", id).Updates(updates).Error
}
...
func UpdateUserPasswordByUsernameDAO(username, newPassword string) error {
	return database.DB.Model(&domain.User{}).Where("username = ?", username).Update("password", newPassword).Error
}
...
func SaveEmailVerificationDAO(ev domain.EmailVerification) error {
	// Remove existing codes for this email first
	database.DB.Where("email = ?", ev.Email).Delete(&domain.EmailVerification{})
	return database.DB.Create(&ev).Error
}

// FindEmailVerificationDAO retrieves a verification code by email and code
func FindEmailVerificationDAO(email, code string) (domain.EmailVerification, error) {
