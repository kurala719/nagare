...
func AddSiteMessageDAO(msg domain.SiteMessage) error {
	return database.DB.Create(&msg).Error
}

// GetSiteMessagesDAO retrieves site messages with filters
func GetSiteMessagesDAO(userID *uint, unreadOnly bool, limit, offset int) ([]domain.SiteMessage, error) {
	var messages []domain.SiteMessage
...
func CountSiteMessagesDAO(userID *uint) (int64, error) {
	var count int64
	query := database.DB.Model(&domain.SiteMessage{})
	if userID != nil {
		query = query.Where("user_id = ? OR user_id IS NULL", *userID)
	} else {
		query = query.Where("user_id IS NULL")
...
func CountUnreadSiteMessagesDAO(userID *uint) (int64, error) {
	var count int64
	query := database.DB.Model(&domain.SiteMessage{}).Where("is_read = 0")
	if userID != nil {
		query = query.Where("user_id = ? OR user_id IS NULL", *userID)
	} else {
		query = query.Where("user_id IS NULL")
...
func MarkSiteMessageAsReadDAO(id uint) error {
	return database.DB.Model(&domain.SiteMessage{}).Where("id = ?", id).Update("is_read", 1).Error
}
...
func DeleteSiteMessageDAO(id uint) error {
	return database.DB.Delete(&domain.SiteMessage{}, id).Error
}
