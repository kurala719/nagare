...
func AddLogDAO(entry domain.LogEntry) error {
	return database.DB.Create(&entry).Error
}

// SearchLogsDAO retrieves logs by filter
func SearchLogsDAO(filter domain.LogFilter) ([]domain.LogEntry, error) {
	query := database.DB.Model(&domain.LogEntry{})
	if filter.Type != "" {
...
func CountLogsDAO(filter domain.LogFilter) (int64, error) {
	query := database.DB.Model(&domain.LogEntry{})
	if filter.Type != "" {
		query = query.Where("type = ?", filter.Type)
	}
...
func ClearLogsDAO(logType string) (int64, error) {
	res := database.DB.Unscoped().Where("type = ?", logType).Delete(&domain.LogEntry{})
