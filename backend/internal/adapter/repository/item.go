...
func GetAllItemsDAO() ([]domain.Item, error) {
...
func SearchItemsDAO(filter domain.ItemFilter) ([]domain.Item, error) {
	query := database.DB.Model(&domain.Item{})
	if filter.Query != "" {
...
func CountItemsDAO(filter domain.ItemFilter) (int64, error) {
	query := database.DB.Model(&domain.Item{})
	if filter.Query != "" {
		query = query.Where("name LIKE ? OR itemid LIKE ? OR hostid LIKE ?", "%"+filter.Query+"%", "%"+filter.Query+"%", "%"+filter.Query+"%")
	}
...
func GetItemByIDDAO(id uint) (domain.Item, error) {
...
func GetItemsByHIDDAO(hid uint) ([]domain.Item, error) {
	var items []domain.Item
...
func GetItemByHIDAndItemIDDAO(hid uint, itemID string) (domain.Item, error) {
	var item domain.Item
...
func AddItemDAO(item domain.Item) error {
	return database.DB.Create(&item).Error
}

// DeleteItemByIDDAO deletes an item by ID
func DeleteItemByIDDAO(id uint) error {
	return database.DB.Delete(&domain.Item{}, id).Error
}

// UpdateItemDAO updates an item by ID
func UpdateItemDAO(id uint, item domain.Item) error {
	return database.DB.Model(&domain.Item{}).Where("id = ?", id).Updates(map[string]interface{}{
		"name":               item.Name,
		"hid":                item.HID,
		"itemid":             item.ItemID,
...
func UpdateItemStatusDAO(id uint, status int) error {
	return database.DB.Model(&domain.Item{}).Where("id = ?", id).Update("status", status).Error
}
