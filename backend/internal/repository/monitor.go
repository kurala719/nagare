...
func GetAllMonitorsDAO() ([]model.Monitor, error) {
...
func SearchMonitorsDAO(filter model.MonitorFilter) ([]model.Monitor, error) {
	query := database.DB.Model(&model.Monitor{})
	if filter.Query != "" {
...
func CountMonitorsDAO(filter model.MonitorFilter) (int64, error) {
	query := database.DB.Model(&model.Monitor{})
	if filter.Query != "" {
		query = query.Where("name LIKE ? OR url LIKE ? OR description LIKE ?", "%"+filter.Query+"%", "%"+filter.Query+"%", "%"+filter.Query+"%")
	}
...
func GetMonitorByIDDAO(id uint) (model.Monitor, error) {
	var monitor model.Monitor
	err := database.DB.First(&monitor, id).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return monitor, model.ErrNotFound
	}
	return monitor, err
}

// GetMonitorByEventTokenDAO retrieves a monitor by event token
func GetMonitorByEventTokenDAO(eventToken string) (model.Monitor, error) {
	var monitor model.Monitor
...
func AddMonitorDAO(m model.Monitor) error {
	return database.DB.Create(&m).Error
}

// DeleteMonitorByIDDAO deletes a monitor by ID
func DeleteMonitorByIDDAO(id int) error {
	return database.DB.Delete(&model.Monitor{}, id).Error
}

// UpdateMonitorDAO updates a monitor by ID
func UpdateMonitorDAO(id int, m model.Monitor) error {
	return database.DB.Model(&model.Monitor{}).Where("id = ?", id).Updates(map[string]interface{}{
		"name":               m.Name,
		"url":                m.URL,
		"username":           m.Username,
		"password":           m.Password,
...
func UpdateMonitorStatusDAO(id uint, status int) error {
	return database.DB.Model(&model.Monitor{}).Where("id = ?", id).Update("status", status).Error
}
...
func UpdateMonitorAuthTokenDAO(id uint, authToken string) error {
	return database.DB.Model(&model.Monitor{}).Where("id = ?", id).Update("auth_token", authToken).Error
}
...
func UpdateMonitorEventTokenDAO(id uint, eventToken string) error {
	return database.DB.Model(&model.Monitor{}).Where("id = ?", id).Update("event_token", eventToken).Error
}
