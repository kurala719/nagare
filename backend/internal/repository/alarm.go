...
func GetAllAlarmsDAO() ([]model.Alarm, error) {
...
func SearchAlarmsDAO(filter model.AlarmFilter) ([]model.Alarm, error) {
	query := database.DB.Model(&model.Alarm{})
	if filter.Query != "" {
...
func CountAlarmsDAO(filter model.AlarmFilter) (int64, error) {
	query := database.DB.Model(&model.Alarm{})
	if filter.Query != "" {
		query = query.Where("name LIKE ? OR url LIKE ? OR description LIKE ?", "%"+filter.Query+"%", "%"+filter.Query+"%", "%"+filter.Query+"%")
	}
...
func GetAlarmByIDDAO(id uint) (model.Alarm, error) {
	var alarm model.Alarm
	err := database.DB.First(&alarm, id).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return alarm, model.ErrNotFound
	}
	return alarm, err
}

// GetAlarmByEventTokenDAO retrieves an alarm by event token
func GetAlarmByEventTokenDAO(eventToken string) (model.Alarm, error) {
	var alarm model.Alarm
...
func UpdateAlarmStatusDAO(id uint, status int) error {
	return database.DB.Model(&model.Alarm{}).Where("id = ?", id).Update("status", status).Error
}
...
func UpdateAlarmAuthTokenDAO(id uint, authToken string) error {
	return database.DB.Model(&model.Alarm{}).Where("id = ?", id).Update("auth_token", authToken).Error
}
...
func UpdateAlarmEventTokenDAO(id uint, eventToken string) error {
	return database.DB.Model(&model.Alarm{}).Where("id = ?", id).Update("event_token", eventToken).Error
}
